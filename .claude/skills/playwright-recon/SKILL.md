...
|---------|----------|-------------------|
| Screen Inventory | {YES/NO} | Understanding flow structure |
| Per-Screen Component Audit | {YES/NO} | Ensuring plan covers every component |
| CSS Mapping Table (per component) | {YES/NO} | Exact Tailwind classes for every step |
| Gradient tables | {YES/NO} | Exact gradient strings |
| Vertical Rhythm tables | {YES/NO} | Spacing between sections |
| Image tables | {YES/NO} | Image dimensions |
| Link inventory tables | {YES/NO} | Ensuring links are `<a>` tags with correct styling |
| States tables (hover/focus) | {YES/NO} | Tailwind hover:/focus: variants |
| Conditional/dynamic content | {YES/NO} | Conditional render logic |
| Responsive CSS Mapping Table | {YES/NO} | Mobile-first implementation |
| Text Content Snapshot | {YES/NO} | Verified by /verify-with-playwright |
```
...
- The recon report feeds into `/plan` as design context
