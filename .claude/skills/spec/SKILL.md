...
**Before drafting the spec, read the following to understand the current state of the project.** Items are listed in priority order. Items 1-3 are mandatory for every spec. Items 4-5 are conditionally mandatory — read them if the spec touches the relevant area. Items 6-10 are conditional — read them when applicable, skip gracefully when they don't exist.
...
8. **External Recon Report** (if this feature replicates a specific live page) — if the feature is "rebuild this existing page" or "replicate this external page," check `_plans/recon/` for a per-page Recon Report (`_plans/recon/{slug}.md`) produced by `/playwright-recon` in default mode. This is distinct from the design-system-wide Reference above: it captures one specific page's per-screen CSS Mapping Tables, gradients, and behavior. If the feature is a replication and no Recon Report exists yet, note in the spec output that the user should run `/playwright-recon {url}` before planning, and reference the report path in the spec's Design Notes so `/plan` can transport it into the plan's Recon Context section.
9. **Existing specs in `_specs/`** — read them to match the depth, tone, and format of specs already written for this project. The Daily Experience spec is the gold standard for thoroughness.
10. **Master spec plan** (if referenced in `$ARGUMENTS` or discoverable) — check for master plan documents in `_plans/_archive/` or `_plans/` (e.g., `_plans/_archive/dashboard-growth-spec-plan-v2.md`). If the current spec is part of a multi-spec feature, the master plan contains shared data models, localStorage keys, integration points, and cross-spec dependencies. Reference these in the spec. If no master plan exists or is relevant, skip this step.
...
- {Reference Design System Reference patterns by name if `_plans/recon/design-system.md` exists}
- {Reference existing components by name when the feature should reuse them}
- {Flag any NEW visual patterns not yet captured in the Design System Reference}
...
- If `_plans/recon/design-system.md` exists, reference specific patterns from it by name (e.g., "use the Hero Section Pattern from the Design System Reference", "match the Card Pattern's exact box-shadow and border-radius")
...
- If the feature introduces patterns NOT yet captured in the Design System Reference, flag them as **new patterns** so `/plan` knows to mark derived values as `[UNVERIFIED]` until verified
- **If this feature replicates a specific page and a Recon Report exists**, cite its path here (e.g., "Recon Report: `_plans/recon/turnaround-calculator.md`"). `/plan` reads this reference and transports the report's CSS Mapping Tables into the plan's Recon Context section. Without the citation, `/plan` won't know the Recon Report exists.
...
- [ ] Design System Reference referenced where applicable (or absence flagged)
...
Design System Reference: {referenced / not found — run /playwright-recon --internal first}
...
```
⚠️  No Design System Reference found. For more accurate UI implementation, run:
...
- `/playwright-recon` — Capture visual specs from live pages (`--internal` for the design system, default mode for an external page this spec replicates)
