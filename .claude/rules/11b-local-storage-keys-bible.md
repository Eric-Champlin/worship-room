## localStorage Key Inventory — Bible Redesign + AI/Audio Caches

This file is the continuation of **[11-local-storage-keys.md](./11-local-storage-keys.md)** and covers:

- Bible Reader keys (`bible:*` and `wr_bible_*`)
- AI Cache (BB-32), Audio Cache (BB-26), Bible Audio Auto-Advance (BB-29), Read-Along Preference (BB-44)
- The Reactive Store Consumption guidance (Pattern A vs Pattern B subscription)

For canonical Worship Room app keys (`wr_*` for auth, mood/activity, social, mutes, dashboard, music, daily hub, content features, prayer wall reactions, challenges, AI Bible chat, local support, PWA, engagement, push notifications) and the `wr_*` naming conventions, see **[11-local-storage-keys.md](./11-local-storage-keys.md)**.

**Prefix conventions for this file:**
...
- `wr_bible_*` — older Bible-related keys that predate the redesign or live alongside `bible:*` for forward-compat reasons.
- `bb*-v1:*` — Cache and preference modules versioned for safe future invalidation. `bb32-v1:` is AI cache (BB-32), `bb26-v1:` is audio cache (BB-26), `bb29-v1:` is auto-advance preference (BB-29), `bb44-v1:` is read-along preference (BB-44).

### Bible Reader

| Key                                       | Type                                                                                                                          | Feature                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `wr_bible_progress`                       | {book: number[]}                                                                                                              | Chapters read per book. **Reactive store (Spec 8B).** Module + hook: `hooks/useBibleProgress.ts` (combines store state and hook in one file). Subscription via `useBibleProgress()` hook (Pattern A — `useSyncExternalStore`). Cross-tab sync via `storage` event listener. `markChapterRead` is no longer auth-gated post-Spec-8B.                                                                                                                                                                                                                                                                                                                       |
...
| `wr_bible_notes` _(deprecated)_           | BibleNote[] (max 200)                                                                                                         | **DEPRECATED.** Pre-redesign verse notes; replaced by `bible:notes` in BB-8. No longer written; ignored on read. Constant `BIBLE_NOTES_KEY` still exported from `constants/bible.ts` for backward-compat. Will be removed in a future cleanup spec.                                                                                          |
...
### Reactive stores across the codebase

This table covers reactive stores documented in BOTH this file and **[11-local-storage-keys.md](./11-local-storage-keys.md)**.

| Storage key             | Store module                        | Subscription pattern                      | Spec      | Documented in |
| ----------------------- | ----------------------------------- | ----------------------------------------- | --------- | ------------- |
| `wr_bible_highlights`   | `lib/bible/highlightStore.ts`       | Inline `subscribe()` (Pattern B)          | BB-7      | 11b           |
| `bible:bookmarks`       | `lib/bible/bookmarkStore.ts`        | Inline `subscribe()` (Pattern B)          | BB-7      | 11b           |
| `bible:notes`           | `lib/bible/notes/store.ts`          | Inline `subscribe()` (Pattern B)          | BB-8      | 11b           |
| `bible:journalEntries`  | `lib/bible/journalStore.ts`         | Inline `subscribe()` (Pattern B)          | BB-11b    | 11b           |
| `wr_chapters_visited`   | `lib/heatmap/chapterVisitStore.ts`  | Inline `subscribe()` (Pattern B)          | BB-43     | 11b           |
| `wr_memorization_cards` | `lib/memorize/store.ts`             | `useMemorizationStore()` hook (Pattern A) | BB-45     | 11b           |
| `bible:streak`          | `lib/bible/streakStore.ts`          | `useStreakStore()` hook (Pattern A)       | BB-17     | 11b           |
| `bible:plans`           | `lib/bible/plansStore.ts`           | Inline `subscribe()` (Pattern B)          | BB-21     | 11b           |
| `wr_bible_progress`     | `hooks/useBibleProgress.ts`         | `useBibleProgress()` hook (Pattern A)     | Spec 8B   | 11b           |
| `wr_prayer_reactions`   | `lib/prayer-wall/reactionsStore.ts` | `usePrayerReactions()` hook (Pattern A)   | Phase 0.5 | 11            |
...
When adding a new reactive store, document the store module path and subscription pattern in this file (or in 11 if it's a `wr_*` non-Bible key) so future consumers know which approach to follow.

---

> Continued from **[11-local-storage-keys.md](./11-local-storage-keys.md)** — see that file for canonical Worship Room app keys (auth, mood/activity, social, mutes, dashboard, music, daily hub, content features, prayer wall reactions, challenges, AI Bible chat, local support, PWA, engagement, push notifications) and the `wr_*` naming conventions.
