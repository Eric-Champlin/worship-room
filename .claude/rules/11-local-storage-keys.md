...
| Key                   | Type                             | Feature                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `wr_prayer_reactions` | `Record<string, PrayerReaction>` | Prayer Wall reactions — `isPraying`, `isBookmarked`, and `isCandle` per prayer (Spec 3.7 added `isCandle`). **Reactive store (Phase 0.5, extended Spec 3.7).** Module: `lib/prayer-wall/reactionsStore.ts`. Hook: `usePrayerReactions()` (Pattern A via `useSyncExternalStore`). Seeded from `getMockReactions()` on first load when storage is empty. **Spec 3.7 migration:** old-shape (3-field) data is default-filled to `isCandle: false` on hydrate and written back; no version key required (additive migration with safe default). In Phase 3 (Spec 3.10) the localStorage adapter swaps for an API adapter without changing the hook surface. |
