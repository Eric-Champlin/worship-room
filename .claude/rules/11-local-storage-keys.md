...
| Key                   | Type                             | Feature                                                                                                                                                                                                                                                                                                                                                                                    |
| --------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `wr_prayer_reactions` | `Record<string, PrayerReaction>` | Prayer Wall reactions — `isPraying`, `isBookmarked`, `isCandle`, and `isPraising` per prayer (Spec 3.7 added `isCandle`; Spec 6.6 Answered Wall added `isPraising`). **Reactive store (Phase 0.5, extended Spec 3.7 + 6.6).** Module: `lib/prayer-wall/reactionsStore.ts`. Hook: `usePrayerReactions()` (Pattern A via `useSyncExternalStore`). Seeded from `getMockReactions()` on first load when storage is empty. **Spec 3.7 + 6.6 migration:** old-shape entries missing `isCandle` and/or `isPraising` are default-filled to `false` on hydrate and written back. No version key required — additive shape with safe `false` defaults makes the migration one-pass and irreversible. Two-stage migration: pre-3.7 data (3 fields) default-fills both `isCandle` and `isPraising`; post-3.7-pre-6.6 data (4 fields with `isCandle` only) default-fills just `isPraising`. In Phase 3 (Spec 3.10) the localStorage adapter swaps for an API adapter without changing the hook surface. |
