...
`BackgroundCanvas` is the canonical inner-page atmospheric layer post-Visual-Rollout. Lives at `frontend/src/components/ui/BackgroundCanvas.tsx`. It renders a 5-stop multi-bloom radial-and-linear gradient on a `min-h-screen overflow-x-clip` root with `data-testid="background-canvas"`, positioning content via `relative z-10` so children sit above the gradient.

**`overflow-x-clip` (NOT `overflow-hidden`) — sticky-safety rule (post-Spec-4.8):** The root uses `overflow-x: clip` to prevent the atmospheric blooms from causing horizontal scrollbars. It MUST NOT use `overflow: hidden` — that establishes a scroll container which traps descendants with `position: sticky` so they scroll with the body instead of clamping to the viewport. This was caught during Spec 4.8 verification: a site-wide regression introduced by Spec 14 (Cinematic Hero Rollout, 2026-05-07) where `overflow-hidden` on `BackgroundCanvas` broke the sticky tab bar on `/daily`, the sticky filter bar on `/grow`, and (when Prayer Wall adopted `BackgroundCanvas`) the dual-filter sticky wrapper on `/prayer-wall`. The fix uses `overflow-x: clip` because `clip` does NOT create a scroll container per the CSS Overflow Module Level 3 spec. **Do not pass `overflow-hidden` or `overflow-x-hidden` via the consumer `className` either** — that would re-introduce the bug. The regression is locked in by `frontend/e2e/sticky-regression.spec.ts`.
...
**Pages using `<BackgroundCanvas>`** (post-Spec-14): DailyHub, BibleLanding, MyBiblePage, BiblePlanDetail, BiblePlanDay, PlanBrowserPage, ReadingPlanDetail, ChallengeDetail, GrowPage (entire page wrap — Spec 14 promoted from body-only), LocalSupportPage (Churches, Counselors, CelebrateRecovery), AskPage, RegisterPage, CreatePlanFlow, **PrayerWall (Spec 14 promoted from `bg-dashboard-dark` outer wrapper; the redundant `overflow-x-hidden` previously passed via `className` was removed post-Spec-4.8 because `BackgroundCanvas` now owns the horizontal-clip constraint via `overflow-x-clip`)**, **MusicPage (Spec 14 — Decision 24 reconciled to Outcome A)**.
...
| `overflow-hidden` on `BackgroundCanvas` root (traps sticky descendants) | `overflow-x-clip` (post-Spec-4.8 sticky-safety fix; see "BackgroundCanvas Atmospheric Layer" section) |
| Passing `overflow-hidden` / `overflow-x-hidden` via consumer `className` to `BackgroundCanvas` | Let `BackgroundCanvas` own the horizontal-clip constraint via `overflow-x-clip` (post-Spec-4.8) |
