...
- **Prompt 3 (Visual Verification):** after any spec that touches a global layout, navbar, or footer
- **Prompt 4 (Dependency & Supply Chain):** monthly, plus whenever a security advisory is announced
- **Prompt 5 (Architecture & Pattern Consistency):** quarterly
...
3. **Prompt 4 — Dependency & Supply Chain.** Independent of Prompt 3 in theory, but easier to run before visual verification because security fixes often require dependency updates that change how pages render.
4. **Prompt 5 — Architecture & Pattern Consistency.** Independent of Prompt 3 but benefits from a clean build and clean tests.
5. **Prompt 3 — Visual Verification.** Last because visual checks against broken code, broken tests, broken dependencies, or inconsistent architecture are wasted effort.
...
Where `YYYY-MM-DD` is the date the run started. If multiple runs happen on the same day, append `-N` (e.g., `deep-review-2026-04-09-2.md`).

The report contains:

- **Header** with date, branch, commit hash, runner identity, total duration
...
- **Don't skip Prompt 4 because dependencies "feel fine."** Supply chain issues are silent until they're catastrophic.
...
| Prompt 3 produces 80+ P1 visual issues | Visual review has been deferred too long | Book a half-day to triage; do not try to fix in one sitting |
| Prompt 4 reports critical CVEs | Ignored security advisories | Stop everything, patch immediately, rerun the protocol from Prompt 1 |
| Prompt 5 finds the codebase has 4 different state management patterns | Architecture has drifted | Document each pattern, propose a unification plan, do not fix in this protocol run |
...
- **Lighthouse CI** for performance baselines (Prompt 3)
- **`license-checker`** for dependency license audit (Prompt 4)
...
| `03-prompt-dependency-and-supply-chain.md` | Prompt 4: vulnerabilities, staleness, bundle impact, licenses. |
| `04-prompt-architecture-and-pattern-consistency.md` | Prompt 5: store patterns, hook patterns, file conventions, error handling. |
| `05-prompt-visual-verification.md` | Prompt 3: full-site Playwright sweep across breakpoints and states. |
| `99-project-specific-overrides.md` | Worship Room specific overrides, key names, file paths, conventions. |

The numbering reflects execution order. Prompt 3 (Visual Verification) runs last despite being the third file in the original protocol design.
