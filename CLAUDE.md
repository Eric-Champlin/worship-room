...
**Frontend regression baseline (post-pre-Phase-7 cleanup, 2026-05-16):** 10,622 pass / 0 persistent fail. The pre-Phase-7 cleanup spec fixed the `warm-empty-states.test.tsx > FriendsPreview "Faith grows stronger together"` assertion (was failing because the same copy renders in both `FriendsPreview` and `WeeklyRecap` per Decision 8; assertion now scopes via `within(friendsCard)` matching the canonical `empty-states.test.tsx` pattern). The prior Spec-13 baseline `Pray.test.tsx` failure is no longer observed; `useNotifications.test.ts > returns notifications sorted newest first` remains a known intermittent flake (~33%) but is not a persistent failure. **Any NEW failing file or any increase in persistent fail count after a Forums Wave spec or Visual Rollout spec lands is a regression.**
