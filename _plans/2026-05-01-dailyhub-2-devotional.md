...
it('reflection question callout matches passage callout Tier 2 idiom', () => {
  const { container } = renderComponent()
  const callouts = container.querySelectorAll('.rounded-xl.border-l-4.border-l-primary\\/60')
...
|------|-------|--------|-----------------|----------------------|
| 1 | Add Button import to DevotionalTabContent.tsx | [COMPLETE] | 2026-05-01 | `frontend/src/components/daily/DevotionalTabContent.tsx` — added `import { Button } from '@/components/ui/Button'` after the FrostedCard import. |
| 2 | Migrate "Meditate on this passage" CTA → subtle Button via asChild | [COMPLETE] | 2026-05-01 | `DevotionalTabContent.tsx` — replaced the rolls-own white-pill `<Link>` className with `<Button variant="subtle" size="md" asChild>` wrapping the existing `<Link>`. Href preserved verbatim. |
| 3 | Migrate reflection body card → FrostedCard accent + eyebrow | [COMPLETE] | 2026-05-01 | `DevotionalTabContent.tsx` — added `variant="accent"` and `eyebrow="Today's reflection"` to the reflection FrostedCard. `p-5 sm:p-8` padding override preserved. Inner reading typography untouched. |
| 4 | Migrate quote card → FrostedCard default (explicit) | [COMPLETE] | 2026-05-01 | `DevotionalTabContent.tsx` — added explicit `variant="default"` to the quote FrostedCard. Behavioral no-op (default value), code-review readability win. |
| 5 | Migrate reflection question card → rolls-own Tier 2 callout (with embedded subtle Button) | [COMPLETE] | 2026-05-01 | `DevotionalTabContent.tsx` — replaced FrostedCard with rolls-own `<div className="rounded-xl border-l-4 border-l-primary/60 bg-white/[0.04] px-5 py-6 sm:px-7 sm:py-7">`. Eyebrow tokens migrated to `tracking-[0.15em] text-white/50 mb-3` (no leading dot). Question paragraph gained `mb-5`; `<div className="mt-5">` wrapper removed. CTA migrated to `<Button variant="subtle" size="sm" type="button" onClick={...}>`. **`ref={questionRef}` preserved on the OUTER `<div className="py-6 sm:py-8">`** — verified post-change via grep + Read. Completion tracking contract intact. |
| 6 | Migrate "Pray about today's reading" CTA → gradient Button (page emotional peak) | [COMPLETE] | 2026-05-01 | `DevotionalTabContent.tsx` — replaced rolls-own white-pill `<button>` with `<Button variant="gradient" size="lg" type="button" onClick={...}>`. customPrompt template literal and `onSwitchToPray` callback preserved verbatim. Wrapping flex container preserved. Verified single gradient Button on the tab via grep. |
| 7 | Migrate Share + Read Aloud action buttons → subtle Buttons | [COMPLETE] | 2026-05-01 | `DevotionalTabContent.tsx` — replaced both rolls-own pills with `<Button variant="subtle" size="md" type="button">`. Icons preserved as `<Share2 size={18} aria-hidden="true">` and `<Volume2 size={18} aria-hidden="true">`. Read Aloud dynamic label preserved. Wrapping flex container preserved. |
| 8 | Test surgery on DevotionalTabContent.test.tsx | [COMPLETE] | 2026-05-01 | `frontend/src/components/daily/__tests__/DevotionalTabContent.test.tsx` — applied 8a (rename Tier 3 → Tier 1, update to `bg-violet-500/[0.08]` + `border-violet-400`), 8b (rename Tier 3 → Tier 1 padding test), 8c (rolls-own Tier 2 selector + tokens), 8d (`tracking-[0.15em]` + `text-white/50`), 8e (`rounded-full` + `bg-white/[0.07]`), 8f (`bg-gradient-to-br` + `from-violet-400`), 8g (eyebrow assertion — NEW), 8h (Tier 2 unification — NEW). Test count: 56 → 58 passing. All behavioral tests preserved. |
| 9 | Documentation update — `09-design-system.md` Tier 1 vs Tier 2 eyebrow distinction | [COMPLETE] | 2026-05-01 | `.claude/rules/09-design-system.md` § "FrostedCard Tier System (Spec T)" — updated Tier 1 bullet (reflection body now accent + eyebrow, saint quote now default explicit), updated Tier 2 bullet (reflection question now also Tier 2), added the "Eyebrow distinction (DailyHub 2)" paragraph documenting the dot-vs-no-dot rule. |
| 10 | Verification (typecheck + tests + lint + build) | [COMPLETE] | 2026-05-01 | `pnpm tsc --noEmit` PASS. `pnpm lint` PASS. `pnpm test` 9383 pass / 1 fail across 725 files — single failure is pre-existing in `src/hooks/__tests__/useFaithPoints.test.ts`, verified by `git stash` + re-run on baseline. `pnpm build` PASS. DevotionalTabContent.test.tsx: 58 pass / 0 fail. **No new regressions.** Manual eyeball review on `/daily?tab=devotional` deferred to user — no Playwright visual regression infrastructure for this spec. |
