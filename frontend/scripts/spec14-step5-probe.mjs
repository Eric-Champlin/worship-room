// Spec 14 Step 5 — verify /grow cinematic mount + reduced motion behavior
...
async function probe(width, height, suffix, opts = {}) {
  ...
  const cinematic = await page.$('[aria-labelledby="grow-heading"] > [data-testid="cinematic-hero-background"]')
  ...
  // Sticky tab bar still present
  const stickyBar = await page.$('.sticky.top-0.z-40')
  results.assertions.push({ name: `[${suffix}] sticky tab bar still pins`, pass: !!stickyBar })
  ...
  await page.screenshot({
    path: `playwright-screenshots/spec14-step5-grow-${suffix}.png`,
    fullPage: false,
  })
