...
// Spec 6.6 — Answered Wall praising-reaction flip. Mirrors applyCandleFlip
// exactly; pendingMutations key is `${prayerId}:praising`.
function applyPraisingFlip(prayerId: string): boolean {
  const current = getCache()[prayerId]
  const wasPraising = current?.isPraising ?? false
  const next: PrayerReaction = {
    prayerId,
    isPraying: current?.isPraying ?? false,
    isBookmarked: current?.isBookmarked ?? false,
...
/** Toggles isPraising for prayerId. Returns the PREVIOUS isPraising value. Spec 6.6 (Answered Wall). */
export function togglePraising(prayerId: string): boolean {
  if (!isBackendPrayerWallEnabled()) {
    const wasPraising = applyPraisingFlip(prayerId)
    writeToStorage(getCache())
    return wasPraising
  }
  ...
  // Pending-mutation guard fires BEFORE the optimistic flip — see
  // togglePraying for the rationale.
  const key = `${prayerId}:praising`
  if (pendingMutations.has(key)) {
    return getCache()[prayerId]?.isPraising ?? false
  }
  ...
  void prayerWallApi
    .toggleReaction(prayerId, 'praising')
    .then((result) => {
      const serverIsPraising = result.state === 'added'
      const cachedIsPraising = getCache()[prayerId]?.isPraising ?? false
      if (serverIsPraising !== cachedIsPraising) {
        forceState(prayerId, { isPraising: serverIsPraising })
      }
    })
    .catch((err) => {
      applyPraisingFlip(prayerId) // rollback
      handleToggleError(err)
    })
    .finally(() => {
      pendingMutations.delete(key)
