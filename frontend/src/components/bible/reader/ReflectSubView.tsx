...
import { useReflectOnPassage } from '@/hooks/bible/useReflectOnPassage'
...
/**
 * 20-verse cap — a defensive guard for over-large selections. Mirrors
 * `ExplainSubView`'s cap exactly.
 */
const MAX_VERSES_PER_REQUEST = 20

/**
 * Sub-view for BB-31 "Reflect on this passage".
 *
 * Follows the CrossRefsSubView / ExplainSubView structural pattern: the
 * component body starts with a plain `<div>` — no back button, no title row,
 * no close X. The sheet chrome is rendered by `VerseActionSheet` around the
 * sub-view. Do NOT follow the NoteEditorSubView pattern (self-rendered
 * chrome) — that causes a double-header bug.
 *
 * The sub-view renders:
 *   1. Subtitle row   — "Reflection for {reference}"
 *   2. Context strip  — Tier 2 scripture callout with the passage text
 *   3. Divider
 *   4. One of: loading / success+body+disclaimer / error
 *
 * `ExplainSubViewLoading` and `ExplainSubViewError` are deliberately reused
 * (not duplicated) per spec §"Sub-view contents" — the loading and error
 * experiences are identical across Explain and Reflect, and a shared import
 * keeps the two features structurally parallel.
 *
 ...
 */
export function ReflectSubView({
  selection,
  onBack: _onBack,
  context: _context,
}: ReflectSubViewProps) {
  ...
  const state = useReflectOnPassage(safeReference, safeVerseText)
  ...
  return (
    <div>
      ...
      <div className="px-4 py-1.5">
        <span className="text-xs text-white/50">Reflection for {reference}</span>
      </div>
      ...
      {effectiveStatus === 'error' && isOverLimit && (
        <ExplainSubViewError
          kind="unavailable"
          message={`Please select ${MAX_VERSES_PER_REQUEST} or fewer verses to reflect on.`}
          onRetry={() => {
