...
export function StreakChip({ streak, atRisk, pendingMilestone, onMilestoneDismissed, onClick }: StreakChipProps) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-full bg-white/[0.06] border border-white/[0.12] px-3 py-1.5 text-sm font-semibold text-white transition-all duration-300 hover:bg-white/[0.09] min-h-[44px] ${glowClass} ${milestoneClass}`}
    >
