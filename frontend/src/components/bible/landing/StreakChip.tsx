...
export function StreakChip({ streak, onClick }: StreakChipProps) {
  ...
  const glowClass = streak.count > 7
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`inline-flex items-center gap-1.5 rounded-full bg-white/[0.06] border border-white/[0.12] px-3 py-1.5 text-sm font-semibold text-white transition-all duration-300 hover:bg-white/[0.09] min-h-[44px] ${glowClass}`}
    >
