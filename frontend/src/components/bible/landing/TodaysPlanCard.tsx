...
export function TodaysPlanCard({ plans }: TodaysPlanCardProps) {
  if (plans.length === 0) {
    return (
      <FrostedCard as="article">
        <Link to="/bible/plans" className="flex flex-col items-center gap-3 text-center">
  ...
  return (
    <FrostedCard as="article">
      <Link to={`/reading-plans/${plan.planId}`} className="block">
      ...
      {extraCount > 0 && (
        <Link
          to="/bible/plans"
          className="mt-3 inline-flex items-center rounded-full bg-white/[0.06] border border-white/[0.12] px-2.5 py-1 text-xs font-medium text-white/60 hover:bg-white/[0.09]"
        >
          +{extraCount} more
        </Link>
      )}
