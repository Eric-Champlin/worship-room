...
// C8: simplified — removed no-op ternary
function renderPage(initialEntries = ['/journal'], state?: unknown) {
  return render(
    <MemoryRouter
      initialEntries={
        state ? [{ pathname: '/journal', state }] : initialEntries
      }
...
describe('Journal', () => {
  ...
  it('shows context banner when arriving from Pray', () => {
    renderPage(['/journal'], { from: 'pray', topic: 'anxiety' })
    expect(
  ...
  it('"Done journaling" shows handoff CTAs', async () => {
    ...
    expect(screen.getByText(/return to daily/i)).toBeInTheDocument()
    expect(screen.getByText('Continue to Meditate')).toBeInTheDocument()
  ...
  it('renders Song Pick section', () => {
    renderPage()
    expect(screen.getByText("Today's Song Pick")).toBeInTheDocument()
  })
