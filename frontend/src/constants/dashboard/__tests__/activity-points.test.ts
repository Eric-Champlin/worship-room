import { describe, it, expect } from 'vitest';
import {
  ACTIVITY_POINTS,
  MULTIPLIER_TIERS,
  MAX_DAILY_BASE_POINTS,
  MAX_DAILY_POINTS,
  ALL_ACTIVITY_TYPES,
  ACTIVITY_DISPLAY_NAMES,
  ACTIVITY_CHECKLIST_NAMES,
} from '../activity-points';

describe('ACTIVITY_POINTS', () => {
  it('has correct values for all 14 activity types', () => {
    expect(ACTIVITY_POINTS.mood).toBe(5);
    expect(ACTIVITY_POINTS.pray).toBe(10);
    expect(ACTIVITY_POINTS.listen).toBe(10);
    expect(ACTIVITY_POINTS.prayerWall).toBe(15);
    expect(ACTIVITY_POINTS.readingPlan).toBe(15);
    expect(ACTIVITY_POINTS.meditate).toBe(20);
    expect(ACTIVITY_POINTS.journal).toBe(25);
    expect(ACTIVITY_POINTS.gratitude).toBe(5);
    expect(ACTIVITY_POINTS.reflection).toBe(10);
    expect(ACTIVITY_POINTS.challenge).toBe(20);
    expect(ACTIVITY_POINTS.localVisit).toBe(10);
    expect(ACTIVITY_POINTS.devotional).toBe(10);
    expect(ACTIVITY_POINTS.intercession).toBe(10);
  });

  it('sums to MAX_DAILY_BASE_POINTS', () => {
    const total = Object.values(ACTIVITY_POINTS).reduce((sum, v) => sum + v, 0);
    expect(total).toBe(MAX_DAILY_BASE_POINTS);
    expect(total).toBe(185);
  });
});

describe('MULTIPLIER_TIERS', () => {
  it('has 4 tiers sorted by minActivities descending', () => {
    expect(MULTIPLIER_TIERS).toHaveLength(4);
    expect(MULTIPLIER_TIERS[0].minActivities).toBe(7);
    expect(MULTIPLIER_TIERS[1].minActivities).toBe(4);
    expect(MULTIPLIER_TIERS[2].minActivities).toBe(2);
    expect(MULTIPLIER_TIERS[3].minActivities).toBe(0);
  });

  it('has correct multiplier values', () => {
    expect(MULTIPLIER_TIERS[0].multiplier).toBe(2);
    expect(MULTIPLIER_TIERS[1].multiplier).toBe(1.5);
    expect(MULTIPLIER_TIERS[2].multiplier).toBe(1.25);
    expect(MULTIPLIER_TIERS[3].multiplier).toBe(1);
  });
});

describe('MAX_DAILY_POINTS', () => {
  it('equals MAX_DAILY_BASE_POINTS × 2x multiplier', () => {
    expect(MAX_DAILY_POINTS).toBe(MAX_DAILY_BASE_POINTS * 2);
    expect(MAX_DAILY_POINTS).toBe(370);
  });
});

describe('ALL_ACTIVITY_TYPES', () => {
  it('contains all 14 activity types', () => {
    expect(ALL_ACTIVITY_TYPES).toHaveLength(14);
    expect(ALL_ACTIVITY_TYPES).toContain('mood');
    expect(ALL_ACTIVITY_TYPES).toContain('pray');
    expect(ALL_ACTIVITY_TYPES).toContain('listen');
    expect(ALL_ACTIVITY_TYPES).toContain('prayerWall');
    expect(ALL_ACTIVITY_TYPES).toContain('readingPlan');
    expect(ALL_ACTIVITY_TYPES).toContain('meditate');
    expect(ALL_ACTIVITY_TYPES).toContain('journal');
    expect(ALL_ACTIVITY_TYPES).toContain('gratitude');
    expect(ALL_ACTIVITY_TYPES).toContain('reflection');
    expect(ALL_ACTIVITY_TYPES).toContain('challenge');
    expect(ALL_ACTIVITY_TYPES).toContain('localVisit');
    expect(ALL_ACTIVITY_TYPES).toContain('devotional');
    expect(ALL_ACTIVITY_TYPES).toContain('intercession');
    expect(ALL_ACTIVITY_TYPES).toContain('quickLift');
  });
});

describe('ACTIVITY_DISPLAY_NAMES', () => {
  it('has display names for all 14 types', () => {
    expect(Object.keys(ACTIVITY_DISPLAY_NAMES)).toHaveLength(14);
    expect(ACTIVITY_DISPLAY_NAMES.mood).toBe('Logged mood');
    expect(ACTIVITY_DISPLAY_NAMES.journal).toBe('Journaled');
    expect(ACTIVITY_DISPLAY_NAMES.reflection).toBe('Evening reflection');
    expect(ACTIVITY_DISPLAY_NAMES.challenge).toBe('Challenge');
    expect(ACTIVITY_DISPLAY_NAMES.localVisit).toBe('Visited local support');
    expect(ACTIVITY_DISPLAY_NAMES.intercession).toBe('Interceded');
  });
});

describe('ACTIVITY_CHECKLIST_NAMES', () => {
  it('has all 14 activity types', () => {
    expect(Object.keys(ACTIVITY_CHECKLIST_NAMES)).toHaveLength(14);
    ALL_ACTIVITY_TYPES.forEach((type) => {
      expect(ACTIVITY_CHECKLIST_NAMES[type]).toBeDefined();
    });
  });

  it('uses imperative form (not past tense)', () => {
    expect(ACTIVITY_CHECKLIST_NAMES.mood).toBe('Log your mood');
    expect(ACTIVITY_CHECKLIST_NAMES.pray).toBe('Pray');
    expect(ACTIVITY_CHECKLIST_NAMES.listen).toBe('Listen to worship');
    expect(ACTIVITY_CHECKLIST_NAMES.prayerWall).toBe('Pray for someone');
    expect(ACTIVITY_CHECKLIST_NAMES.readingPlan).toBe('Complete a reading');
    expect(ACTIVITY_CHECKLIST_NAMES.meditate).toBe('Meditate');
    expect(ACTIVITY_CHECKLIST_NAMES.journal).toBe('Journal');
    expect(ACTIVITY_CHECKLIST_NAMES.localVisit).toBe('Visit local support');
    expect(ACTIVITY_CHECKLIST_NAMES.intercession).toBe('Pray for someone (comment)');
  });
});
