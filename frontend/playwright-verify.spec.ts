...
test('Flow 1: /bible/my — logged-out experience + banner visibility', async ({ page }) => {
  ...
  await expect(signInLink.locator('button')).toBeVisible()

  // 5. NO auth gate shell / "Get Started" CTA
  await expect(page.getByText(/Get Started/i)).not.toBeVisible()
...
test('Flow 5: Quick stats cards — FrostedCard chrome (border-white/[0.12] background)', async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem('wr_auth_simulated', 'true')
    localStorage.setItem('wr_user_name', 'Eric')
    // Seed some highlight/bookmark data so stat cards show non-zero counts
    localStorage.setItem('wr_bible_highlights', JSON.stringify([
      { id: 'h1', book: 'John', chapter: 3, verse: 16, color: 'yellow', createdAt: Date.now() }
    ]))
    localStorage.setItem('bible:bookmarks', JSON.stringify([
      { id: 'bk1', book: 'John', chapter: 3, verse: 16, createdAt: Date.now(), label: '' }
    ]))
  })
  ...
  // The stat cards are FrostedCard components — check for border-white/[0.12] style
  // FrostedCard renders with bg-white/[0.07] backdrop-blur-sm border border-white/[0.12]
  // They should NOT have bg-primary class (old style)
  const statCards = page.locator('[class*="min-w-\\[100px\\]"]')
  const count = await statCards.count()
  expect(count).toBeGreaterThan(0)

  // Verify no bg-primary on stat cards
  for (let i = 0; i < Math.min(count, 5); i++) {
    const cls = await statCards.nth(i).getAttribute('class')
    expect(cls).not.toContain('bg-primary')
  }

  console.log(`✅ Flow 5: ${count} stat cards found without bg-primary`)
})
...
test('Flow 7: Banner inline position — copy and Sign In on same row at desktop', async ({ page }) => {
  ...
  const signInBtn = page.locator('a[href="/?auth=login"] button')
...
test('Flow 12: /bible/my logged-out — personal layer (heatmap, memorization) visible', async ({ page }) => {
  ...
  // Verify "Get Started — It's Free" auth shell is NOT rendered
  const getStarted = page.locator('a').filter({ hasText: /Get Started/i })
