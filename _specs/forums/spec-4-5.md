# Forums Wave: Spec 4.5 — Devotional Discussion Post Type

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 4.5
...
The discussion composer surface is `/prayer-wall`. PrayerCard chrome (violet wash, MessagesSquare icon) and the new `<ScriptureChip>` propagate to every site that renders a PrayerCard. The chip is a `<Link>` that navigates into the Bible reader. QOTD responses (existing `postType='discussion'` posts with `qotdId` set) also pick up the new chrome — that is intentional and verified for non-regression on the QotdBadge.

- `/prayer-wall` — PrayerWall.tsx (composer + feed). Production composer entry for discussion ships in Spec 4.7 (Composer Chooser); 4.5 exposes the variant via the existing `?debug-post-type=discussion` query param introduced by 4.3 for `/verify-with-playwright` only, marked `REMOVE-IN-4.7` in the code.
- `/prayer-wall/:id` — PrayerDetail.tsx (single-card render; primary surface for ScriptureChip placement testing)
...
- `/my-prayers` — MyPrayers.tsx (authenticated user's own prayer list; requires auth)
- `/bible/{book.slug}/{chapter}?verse={n}` — ScriptureChip's navigation target (the existing Bible reader URL pattern from R12). 4.5 does NOT modify the Bible reader; the chip merely deep-links into it.

All routes except `/prayer-wall/dashboard` and `/my-prayers` are public. There are no new routes; this spec only modifies what already-existing routes render when a post has `postType='discussion'` (or any `scriptureReference` set, in the chip's case) and adds two new frontend components mounted within those existing routes. Backend delta is zero (per MPD-4 in the brief).
