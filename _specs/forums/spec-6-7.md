# Forums Wave: Spec 6.7 — Shareable Testimony Cards

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 6.7 (`round3-phase06-spec07-shareable-testimony-cards`)
**Brief Source:** `_plans/forums/spec-6-7-brief.md` (preserved verbatim below)
**Branch:** `forums-wave-continued` (per W1 — CC never alters git state; Phase 6 specs accumulate on this branch — see commits `f9f2652 Spec 6-6`, `46d70a7 Spec 6-5`, `d4ccbca Spec 6-4`)
**Date:** 2026-05-14
**Phase:** 6 (Slow Sanctuary / Quiet Engagement Loop) — **FINAL spec in the Phase 6 wave**
**Size:** L (Large)
**Risk:** Medium (master plan); brief concurs
**Tier:** High — share-as-image pipeline is already shipped and de-risked by 6.1; the architecture decision (shared-lib extraction vs. parallel) and brand-visual review keep it above Medium

---

## Affected Frontend Routes

6.7 modifies `InteractionBar.tsx` or `ShareDropdown.tsx` (TBD per Plan-Recon R5/D-AffordancePlacement). Both surface on every Prayer Wall family route, but the testimony-only render-gate (Gate-G-TESTIMONY-ONLY) means the new "Share as image" affordance appears only on `post_type === 'testimony'` posts. The routes below are the candidate surfaces `/verify-with-playwright` should cover — verifying both that the affordance appears on testimony posts AND that it's absent on every other post type.

- `/prayer-wall` (main feed — testimony posts in feed surface the affordance)
- `/prayer-wall/:id` (PrayerDetail — testimony detail surfaces the affordance + the share modal)
- `/prayer-wall/user/:id` (PrayerWallProfile — testimony posts in a user profile surface the affordance)
- `/prayer-wall/dashboard` (user's own testimonies)
- `/prayer-wall/answered` (Answered Wall — if any answered post is a testimony post-type, the affordance surfaces there too; verification confirms this is consistent with Gate-G-TESTIMONY-ONLY)

---

## Spec-time Recon Summary (verified on disk 2026-05-14)

Brief's R1-R5 verified during brief authoring (Section 5). This summary re-confirms key items on disk at spec-creation time and flags items deferred to plan-time (R6-R10).

| Item | Status | Evidence |
|---|---|---|
| 6.1's `PrayerReceiptImage.tsx` (the reference design component) | ✅ | `frontend/src/components/prayer-wall/PrayerReceiptImage.tsx` — off-screen capture target at 1080×1080, `position: fixed; left: -99999px`, explicit `backgroundColor: '#08051A'` |
| 6.1's `PrayerReceipt.tsx` (the orchestration target for R6 extraction decision) | ✅ | `frontend/src/components/prayer-wall/PrayerReceipt.tsx` — `document.fonts.ready` wait, html2canvas capture with `scale: 2`, Web Share API + download fallback |
| `testimony` post-type enum value | ✅ | `frontend/src/types/api/prayer-wall.ts` — confirmed as part of the post-type union (R4) |
| `InteractionBar.tsx` (potential modify target) | ✅ | `frontend/src/components/prayer-wall/InteractionBar.tsx` — exists and shipped through Spec 6.6's `showPraising` extension |
| `ShareDropdown.tsx` (preferred modify target per D-AffordancePlacement) | ✅ | `frontend/src/components/prayer-wall/ShareDropdown.tsx` — exists and is currently consumed by InteractionBar |
| `html2canvas` dependency | ✅ | Already in use by 6.1; in `package.json` per Eric's standing notes (R-prereq) |
| `frontend/src/lib/prayer-wall/imageGen.ts` (conditional file to create per R6) | ⬜ Not present | Confirms the extraction decision is real — the file does NOT yet exist; R6 picks whether 6.7 creates it |
| 6.6 (Answered Wall — hard prereq) merged | ✅ | Tracker line 167; commit `f9f2652 Spec 6-6` on `forums-wave-continued` (2026-05-14) |
| 6.1 (Prayer Receipt — essential reuse target) shipped | ✅ | Tracker line 161; share-as-image pipeline live |

**Items deferred to plan-time recon (R6–R10 from Section 5 of the brief):**

- **R6 (CENTRAL DECISION):** shared-lib EXTRACTION vs. PARALLEL pattern. Plan reads `PrayerReceipt.tsx` orchestration in full and decides. Brief leans extraction-with-regression-gate; parallel is legitimate if extraction is too entangled.
- **R7:** preview modal vs. direct-share flow — match 6.1's pattern or add a preview step.
- **R8:** long-content strategy — confirm against testimony post-type max content length.
- **R9:** whether to mirror 6.1's advisory rate-limit.
- **R10:** the exact mechanism the existing PrayerCard uses to resolve "Anonymous" vs. display name — `TestimonyCardImage` MUST route through it (load-bearing for Gate-G-ANON-ATTRIBUTION).

---

## Master Plan vs. Brief: Three Items Requiring Plan-Time Reconciliation

The master plan stub (lines 5826–5858 of `_forums_master_plan/round3-master-plan.md`) lists three acceptance criteria the brief does NOT explicitly address. They may be intentionally out of scope for 6.7 OR they may be inherited PII/safety requirements the brief overlooked. **The plan MUST explicitly resolve each as either in-scope (and add the implementation work) or out-of-scope (with rationale).** Do NOT silently drop them.

1. **Pre-share confirmation modal (master plan acceptance criterion):** "before generating the PNG, show a one-time-per-user (sticky-dismissible) modal explaining 'Once you share this image, it cannot be unshared. People who receive it can save it forever, even if you later delete the original testimony.' User must explicitly confirm. Modal does NOT show for subsequent shares from the same user (preference stored in `wr_settings.dismissedShareWarning`)." The brief is silent on this. Plan must decide: include, exclude (with rationale), or defer.

2. **Avatar URL stripping (master plan acceptance criterion):** "Image does NOT include the user's avatar URL when avatar is from an external/uploaded source (only renders initials in a stylized circle) — avatar URLs may contain identifying user IDs that leak in URL inspection of the PNG metadata." The brief addresses display-name anonymity (Gate-G-ANON-ATTRIBUTION) but does NOT address avatar URL handling. Plan must decide whether anonymous-attribution extends to avatar rendering.

3. **PNG metadata stripping (master plan acceptance criterion):** "PNG metadata stripped of EXIF, generation timestamp truncated to date (no time-of-day fingerprinting)." The brief mentions metadata in Gate-G-ANON-ATTRIBUTION ("not in any metadata 6.7 controls") but does NOT specify EXIF stripping or timestamp truncation. Plan must decide whether the brief's "metadata" coverage is meant to subsume these or whether they are additional concrete asks. Note: the master plan elsewhere refers to "Spec 6.7's PII-stripping infrastructure" being reused by later specs (e.g., Spec 6.8 Verse-Finds-You at line 5941, the post-image upload spec at line 4275), implying 6.7 was originally scoped to BUILD this infrastructure. Dropping it silently would orphan downstream specs that reference it.

**These three items raise the test count target.** The master plan acceptance criterion says "At least 12 tests (includes 2 new tests for the share warning and metadata stripping)" — the brief's ~15-test plan does NOT explicitly include warning-modal or metadata-stripping tests. Plan-time test inventory must reconcile.

---

## Brief (preserved verbatim)

The remainder of this file is the verbatim content of `_plans/forums/spec-6-7-brief.md`. The header and recon summary above are this skill's additions; everything below is Eric's authored brief.
