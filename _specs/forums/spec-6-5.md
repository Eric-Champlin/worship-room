# Forums Wave: Spec 6.5 — Intercessor Timeline

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 6.5 (`round3-phase06-spec05-intercessor-timeline`)
**Brief Source:** `_plans/forums/spec-6-5-brief.md` (preserved verbatim below as Sections 1–16)
**Branch:** `forums-wave-continued` (per W1 — CC never alters git state)
**Date:** 2026-05-13
**Tier:** xHigh (public-facing privacy surface; brand voice critical; backend + frontend; performance budget on hot feed path; direct prerequisite for 6.6)

---

## Affected Frontend Routes

`<IntercessorTimeline />` is rendered inside `PrayerCard.tsx`, which appears on every route that displays prayer cards. The summary line lives below post content on each card; tap-to-expand is per-card.
...
|---|---|---|
| `backend/src/main/java/com/worshiproom/post/` package exists with rich Post infrastructure | ✅ | `PostController`, `PostService`, `PostRepository`, `PostSpecifications`, `PostMapper`, `PostVisibility`, `PostVisibilityConverter` all present |
| `post_reactions` table | ✅ | Liquibase changeset `2026-04-27-016-create-post-reactions-table.xml` |
| `PrayerReceiptService.java` + sibling classes | ✅ | `PrayerReceiptController`, `PrayerReceiptService`, `PrayerReceiptReadRateLimit{Service,Config,Exception}`, `PrayerReceiptShareRateLimit{Service,Config,Exception}`, `PrayerReceiptForbiddenException` |
| `DisplayNameResolver.java` + preference enum + converter | ✅ | `backend/src/main/java/com/worshiproom/user/DisplayNameResolver.java`, `DisplayNamePreference.java`, `DisplayNamePreferenceConverter.java` |
| `IntercessorController.java` / `IntercessorService.java` | ⬜ (does NOT exist — this spec creates it) | not in `post/` directory |
| `PrayerCard.tsx` | Assumed present per brief R4 (frontend recon deferred to plan-time per Step 4 reconnaissance scope) | brief R4 |
| Spec file `_specs/forums/spec-6-5.md` | ⬜ before this run | safe to create |
| Prereq spec files exist on disk | ✅ | `spec-6-1.md`, `spec-6-2.md`, `spec-6-2b.md`, `spec-6-3.md`, `spec-6-4.md` present |

**Items deferred to plan-time recon (R5–R10 per brief Section 5):**
- R5: `post_reactions` exact schema (column names, presence of per-reaction anonymous flag, timestamp precision)
- R6: `DisplayNameResolver` public API signature
- R7: `PrayerReceiptService` end-to-end privacy logic (D-PrivacyContrast comparison)
- R8: Existing `PostVisibility` enforcement helper (or extract one)
- R9: Feed query mechanism for `intercessorSummary` field (EXPLAIN ANALYZE required)
- R10: `PrayerReceiptReadRateLimitService` bucket4j pattern to mirror

**Prerequisite status (from `spec-tracker.md` summary in brief Section 16):**
- 6.1 (Prayer Receipt) — ✅ shipped
- 6.2 / 6.2b (Quick Lift / Faithful Watcher) — ✅ shipped
- 6.3 (Night Mode) — ✅ shipped
- 6.4 (3am Watch v1) — ⏳ in progress; brief mandates 6.5 executes AFTER 6.4 merges (no concurrent execution — both touch `PrayerCard` render path)
