# Forums Wave: Spec 7.5 — Local Support Bridges on Mental Health Posts

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 7.5 (lines 6449–6466, tracker row 97)
...
**Size:** M
**Risk:** Medium
**Prerequisites:** Spec 7.4 (✅ shipped per tracker row 96)
**Tier:** Standard

**Brief Source:** `_plans/forums/spec-7-5-brief.md`, authored 2026-05-16 against the live master plan stub + live code recon.
...
**Risk rationale:** The spec is small in code but lives in **load-bearing anti-pressure territory**. The Mental Health category is governed by Gate-G-MH-OMISSION HARD (Spec 6.6 master plan), which excludes Mental Health from the Answered Wall's category filter because *"healing is rarely linear, and a filter chip on a celebration"* implies pressure on people in that territory. 7.5 is the inverse — surfacing local-support resources WHEN a user is composing — which is complementary, NOT contradictory. But the brief must respect the broader anti-pressure design ethos. Risk also comes from: (a) when exactly to render the bridge (composer state observation), (b) session-scoped dismissal semantics, (c) ensuring the bridge doesn't pressure users to seek counseling when they don't need it.

---

## Spec-Time Recon Addendum (2026-05-16)

All brief recon verified on disk at spec time. Three findings worth surfacing for plan-recon:

**1. CrisisBanner.tsx is the EXACT canonical pattern to mirror.** Verified at `frontend/src/components/daily/CrisisBanner.tsx:44-50` — `<Link to="/local-support/counselors" className="text-sm font-semibold text-primary underline">Find a counselor near you &rarr;</Link>`. Note that CrisisBanner uses `text-primary` (which is the deprecated text-button color on dark surfaces per `09-design-system.md` § "Text-Button Pattern (Spec 10A WCAG AA fix)"). Plan-recon should **use `text-violet-300 hover:text-violet-200`** (the post-Spec-10A canonical) for the bridge's link styling, NOT verbatim copy CrisisBanner's `text-primary` which is on the deprecated list. The MPD-3 styling description ("link styled as muted text-primary underline (mirrors CrisisBanner.tsx:46-48)") needs this Spec-10A correction during plan-recon.

**2. `selectedCategory` state confirmed at `InlineComposer.tsx:243`.** `useState<PrayerCategory | null>(null)`, set via category-pill onClick at line 642 (`setSelectedCategory(cat)`). Type `PrayerCategory` is the canonical kebab-case literal union including `'mental-health'`. Reset to `null` on submit/cancel (line 433, 481). MPD-1 condition is implementable directly from this state — no new wiring needed.

**3. Reset-on-submit semantics confirmed.** `InlineComposer.tsx:433` calls `setSelectedCategory(null)` on successful submit; line 481 same on cancel. Combined with MPD-1's condition (`selectedCategory === 'mental-health'`), submission naturally hides the bridge. Combined with sessionStorage dismissal, the bridge does NOT re-render after submit even if the user opens a new composer in the same session. MPD-4 + R7 align.

**No other recon overrides.** All other brief claims verified:
- `'mental-health'` is the canonical category key at `constants/prayer-categories.ts:2,10`.
- `/local-support/counselors` route active at `App.tsx:311`.
- `LOCAL_SUPPORT_LINKS` constant at `LocalSupportDropdown.tsx:6-10`.
- Session-scoped dismissal pattern shipped in `welcome-back-storage.ts:36-56` + `surprise-storage.ts:95-110`.
- Gate-G-MH-OMISSION HARD rationale documented at `AnsweredCategoryFilter.tsx:10-22` + `constants/answered-wall-copy.ts:91-97`.
...
| `CrisisBanner.tsx` already implements the EXACT pattern this spec wants (link to /local-support/counselors with copy) | ✅ | `frontend/src/components/daily/CrisisBanner.tsx:44-48` — `<Link to="/local-support/counselors" className="text-sm font-semibold text-primary underline">`. Copy: *"Find a counselor near you"* (from test at `CrisisBanner.test.tsx:59`). **This is the canonical pattern 7.5 should mirror, EXCEPT swap `text-primary` for `text-violet-300 hover:text-violet-200` per Spec-10A WCAG AA fix — see Recon Addendum item 1.** |
...
| Composer state resets on submit / cancel | ✅ | `InlineComposer.tsx:433` (`setSelectedCategory(null)` on submit) + `:481` (same on cancel). MPD-1's condition naturally hides the bridge on submit |
...
3. **MPD-3 link styling needs Spec-10A correction.** Brief says "muted text-primary underline (mirrors CrisisBanner.tsx:46-48)" — but `text-primary` on dark surfaces is on the post-Spec-10A deprecated list. Plan-recon uses `text-violet-300 hover:text-violet-200` instead. See Recon Addendum item 1.
...
- Link styled as muted **`text-violet-300 hover:text-violet-200`** underline (Spec-10A canonical text-button on dark — NOT `text-primary` which CrisisBanner uses but is deprecated post-10A). See Recon Addendum item 1.
...
**MPD-7.** Auth-agnostic. Unauthenticated users composing a Mental Health prayer request also see the bridge. Counseling resources are public goods (matches Decision 12 — Spec 5 from `02-security.md` § "Auth Gating Strategy": Local Support is intentionally public for crisis-adjacent moral-imperative reasons).
...
- **Gate-G-LINK-COLOR-SPEC-10A.** The bridge link uses `text-violet-300 hover:text-violet-200` (Spec-10A canonical text-button on dark surfaces) — NOT `text-primary` (which CrisisBanner uses but is on the post-Spec-10A deprecated list per `09-design-system.md` § "Deprecated Patterns"). See Recon Addendum item 1.
...
- Visual snapshot or DOM structure check (matches quiet treatment per MPD-3, including Spec-10A link color)
...
- Unauthenticated users see the bridge too — counseling info should not be auth-gated. Per MPD-7 + Decision 12 (Spec 5 in `02-security.md` § "Auth Gating Strategy").
...
## Acceptance Criteria (from master plan Spec 7.5)
...
- **`/spec-forums`:** Produces the actual spec file at `_specs/forums/spec-7-5.md` from this brief. ← (this file)
...
- **`/code-review`:** Standard pass. Specifically check Gate-G-MH-OMISSION-RESPECTED (the spec does not add any Mental Health filter chip or feed surface), Gate-G-NO-CRISIS-UI-CONFLICT, Gate-G-COPY-PASSES-BRAND-VOICE, Gate-G-LINK-COLOR-SPEC-10A.
...
- `_forums_master_plan/round3-master-plan.md` Spec 7.5 stub (lines 6449–6466)
- `_plans/forums/spec-7-5-brief.md` — source brief
- `frontend/src/components/prayer-wall/InlineComposer.tsx:243,642,433,481` — `selectedCategory` state + setter + reset call sites
...
- `frontend/src/components/daily/CrisisBanner.tsx:44-48` — **canonical pattern to mirror** (link to /local-support/counselors with subtle styling); EXCEPT swap `text-primary` for `text-violet-300 hover:text-violet-200` per Spec-10A
...
- `_specs/forums/spec-7-1.md`, `_specs/forums/spec-7-2.md`, `_specs/forums/spec-7-3.md`, `_specs/forums/spec-7-4.md` — sibling Phase 7 specs
- `.claude/rules/09-design-system.md` § "Text-Button Pattern (Spec 10A WCAG AA fix)" — `text-violet-300 hover:text-violet-200` canonical
- `.claude/rules/09-design-system.md` § "Deprecated Patterns" — `text-primary` text-button on dark surfaces is deprecated
- `.claude/rules/02-security.md` § "Auth Gating Strategy" → "Decision 12 — Spec 5" — Local Support is intentionally public (justifies MPD-7)
- `.claude/rules/CRITICAL_NO_AI_AUTO_REPLY.md` — Gate-G-NO-NEW-BACKEND-CALLS extends here (no LLM invocation on Mental Health user content; 7.5 doesn't touch this but is in adjacent territory)
