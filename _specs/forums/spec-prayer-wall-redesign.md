# Forums Wave: Prayer Wall Redesign (Side Quest)

**Master Plan Reference:** N/A — this is a SIDE QUEST. It does NOT appear in `_forums_master_plan/round3-master-plan.md` or `_forums_master_plan/spec-tracker.md`. Eric tracks completion manually.
**Brief Source:** `_plans/forums/prayer-wall-redesign-brief.md` (preserved verbatim below as Sections 1–16)
**Branch:** `forums-wave-continued` (per Section 1 + W1 — CC never alters git state; this side quest continues the existing wave branch rather than creating a new one)
**Date:** 2026-05-13
**Tier:** xHigh (brand-defining surface; reducing shipped functionality — Night Mode palette; load-bearing 3-column layout; responsive complexity across three breakpoints)
**Size:** L
**Risk:** Medium
...
This redesign restructures the Prayer Wall page container and relocates the Night Mode chip from the Prayer Wall hero to the global page header. Routes touched:

- `/prayer-wall` (primary surface — 3-column grid wrapper, sidebar contents, category bar redesign, hero/CTA repositioning)
- `/prayer-wall/:id` (PageShell wrapper inherits the redesigned global header with NightModeBadge; card-internal rendering unchanged)
- `/prayer-wall/user/:id` (same — inherits header changes; card-internal rendering unchanged)
- `/prayer-wall/dashboard` (same — inherits header changes)
- `/settings` (Night Mode preference UX updated to align with the chip cycle semantics Off / Auto / On; "(always on)" copy removed)
- **Global** — any route that mounts the existing top-nav component receives the new NightModeBadge mount in the user-state slot. Plan-time R2 picks the path; current candidate: `frontend/src/components/Navbar.tsx`.

Additional non-route impact: `frontend/src/index.css` changes are global (removing `[data-night-mode='on']` palette, removing the brown `#1f1a17` gradient at line 188, adding `.prayer-wall-grid` responsive rules, fixing QOTD card alpha). Plan-time R1 verifies no other page consumes the palette block.

---

## Spec-time Recon Summary (verified on disk 2026-05-13)

| Item | Status | Evidence |
|---|---|---|
| `frontend/src/pages/PrayerWall.tsx` | ✅ (44 KB; most-modified file in the wave per brief) | file exists; will be wrapped in 3-column CSS grid |
| `frontend/src/hooks/useNightMode.ts` | ✅ (1.9 KB) | file exists; D-PreserveHook mandates ZERO changes |
| `frontend/src/hooks/useWatchMode.ts` | ✅ (2.9 KB) | file exists; composes useNightMode; W5 regression test required |
| `frontend/src/constants/night-mode-copy.ts` | ✅ (3.5 KB) | file exists; D-PreserveCopyVariants mandates ZERO changes |
| `frontend/src/index.css` | ✅ (17 KB) | file exists; brown gradient confirmed at line 188 (`linear-gradient(135deg, var(--pw-night-bg-base) 0%, #1f1a17 50%, var(--pw-night-bg-base) 100%) !important`); palette override block runs lines 159–188 under `[data-night-mode='on']` selector |
| `frontend/src/pages/Settings.tsx` | ✅ (9.3 KB) | file exists; Night Mode preference UX needs alignment per R5 |
| `frontend/src/components/prayer-wall/NightWatchChip.tsx` | ✅ | file exists; renames to NightModeBadge per D-ChipRename; "always on" copy confirmed at lines 25 + 52 |
| `frontend/src/components/prayer-wall/__tests__/NightWatchChip.test.tsx` | ✅ | tests exist; will migrate to NightModeBadge.test.tsx; two existing test cases explicitly assert "(always on)" copy and will be inverted |
| Existing top-nav component | ✅ | `frontend/src/components/Navbar.tsx` is the canonical candidate (no `components/layout/` directory exists); plan-time R2 confirms whether top nav stays as thin header or is replaced by left sidebar at desktop |
| Existing prayer-wall component directory | ✅ | `frontend/src/components/prayer-wall/` exists with 40+ files (CrisisResourcesBanner, IntercessorTimeline, WatchIndicator, PrayerCard, etc. all present per W-PRESERVE list) |
| New components to create (NightModeBadge, QotdSidebar, LocalSupportPromo, CommunityGuidelinesCard, CategoryFilters, PrayerWallLeftSidebar, PrayerWallRightSidebar) | ⬜ (do NOT exist — this spec creates them) | confirmed absent via directory listing |
| `localStorage.wr_prayer_wall_guidelines_dismissed` | ⬜ (new key — this spec introduces it) | needs documentation in `.claude/rules/11-local-storage-keys.md` per Section 10 |
| Phase 6 prereqs (6.1, 6.2, 6.2b, 6.3, 6.4) | ✅ shipped | spec files all present in `_specs/forums/` |
| Phase 6 prereq (6.5 Intercessor Timeline) | ⏳ in progress (per brief; must merge first) | `spec-6-5.md` present; execution gate per Section 16 |
| `_forums_master_plan/spec-tracker.md` entry | ⬜ N/A (side quest — no tracker entry) | per Section 1 |

**Items deferred to plan-time recon (R1–R6 per brief Section 6):**

- **R1:** Brown gradient source confirmation. Spec-recon points to `#1f1a17` in `index.css:188`. Plan must verify this is the ONLY brown source and no other page (Daily Hub, Bible, Music, etc.) consumes the `[data-night-mode='on']` block.
- **R2:** Top-nav structure. Spec-recon identifies `frontend/src/components/Navbar.tsx` as the canonical candidate. Plan reads it to pick: does the desktop left sidebar REPLACE the top nav (top nav hidden ≥ 1280px), or does the top nav stay as a thin header with sidebar adding ADDITIONAL nav surface? Brief recommends the latter — plan confirms.
- **R3:** Chip explainer popover discoverability mechanism. Plan picks (a) tap-and-hold, (b) secondary "i" icon, or (c) first-time-only popover. Brief recommends (b).
- **R4:** QOTD card current CSS. Plan identifies the background CSS variable in use, current alpha value, and picks new alpha (brief recommends 0.85 but consistency with other cards wins over the specific number).
- **R5:** Settings page Night Mode toggle UX. Plan reads `Settings.tsx` to align with chip cycle semantics; brief recommends keeping Settings as the canonical edit surface and chip as a shortcut.
- **R6:** Existing CSS variable name inventory. Plan reads `index.css` for `--bg`, `--text`, `--accent` family names; new components MUST reuse, not parallel-name.

**Prerequisite status (from brief Section 16):**

- 6.1 Prayer Receipt — ✅ shipped
- 6.2 / 6.2b Quick Lift / Faithful Watcher — ✅ shipped
- 6.3 Night Mode — ✅ shipped (this redesign REDUCES it; preserves hook + 8 copy variants + chip; removes CSS palette override)
- 6.4 3am Watch v1 — ✅ shipped (Watch consumes `useNightMode()`; D-PreserveHook is load-bearing)
- 6.5 Intercessor Timeline — ⏳ in progress; brief mandates this side quest executes AFTER 6.5 merges to master (no concurrent execution — both touch `PrayerWall.tsx` and the `PrayerCard` render path)
...
**Spec-recon update (2026-05-13):** Confirmed — the brown comes from `#1f1a17` at `frontend/src/index.css:188`, inside the `[data-night-mode='on']` palette block running lines 159–188. Removing the block per D-RemovePaletteOverride deletes the brown. Plan still verifies no OTHER page consumes the block (no separate brown sources observed in spec-recon).
...
**Spec-recon update (2026-05-13):** Canonical candidate path is `frontend/src/components/Navbar.tsx` (no `components/layout/` directory exists). Plan-time reads this file.
...
- The existing top-nav component (plan-recon R2 identifies path — spec-recon points to `frontend/src/components/Navbar.tsx`) — add NightModeBadge mount point in user-state slot
