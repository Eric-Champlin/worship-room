# Forums Wave: Spec 4.6b — Image Upload for Testimonies & Questions

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 4.6b
...
The image upload affordance lives in the testimony and question composers; the resulting `<PostImage>` component (and its `<ImageLightbox>` overlay) renders inside `PrayerCard`, which is consumed at every Prayer Wall surface. The composer affordance ships behind the established per-type composer infrastructure (`composerCopyByType` extended with `showImageUpload?` and `imageUploadHelperText?` per MPD-3 / D11). The `<PostImage>` component is decoupled from `postType` (W21) — it renders whenever `prayer.image` is non-null, so any feed surface that already renders PrayerCard inherits it without additional wiring.

- `/prayer-wall` — PrayerWall.tsx (composer + main feed). Testimony and question composer affordance ('Add a photo' button, drag-and-drop, alt-text input, Remove button) is gated by `composerCopyByType[postType].showImageUpload === true`. Mixed feed renders PostImage on testimony/question cards interleaved with non-image cards (prayer_request, discussion, encouragement chrome unchanged).
- `/prayer-wall/:id` — PrayerDetail.tsx (single-card render). PostImage renders for image-bearing testimonies/questions; tapping opens ImageLightbox. No composer here, so no upload affordance.
- `/prayer-wall/dashboard` — PrayerWallDashboard.tsx (auth-gated multi-tab feed). PostImage renders on every tab that surfaces image-bearing posts; lightbox available everywhere.
- `/prayer-wall/user/:id` — PrayerWallProfile.tsx (author profile feed; public). PostImage renders inside the per-author feed.
- `/my-prayers` — MyPrayers.tsx (auth-gated; user's saved/bookmarked prayers). PostImage renders on bookmarked image-bearing posts.

The upload endpoint itself (`POST /api/v1/uploads/post-image`) has no UI route; it is consumed by the composer's `prayerWallApi.uploadImage(file)` client method. There are no new routes added by 4.6b. Backend delta: net-new `com.worshiproom.upload` package (controller, service, image processing, rate limit, scheduled cleanup, exception classes), two new nullable columns on `posts` (`image_url`, `image_alt_text`) via Liquibase, plus modifications to `PostService.createPost`, `PostMapper`, `PostDto`, `CreatePostRequest`, and `PostExceptionHandler` to carry the image-claim flow. Reads use presigned-GET URLs constructed at PostMapper serialization time (W10) — no read-through proxy.
