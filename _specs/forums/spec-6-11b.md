# Forums Wave: Spec 6.11b — Live Presence Component

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 6.11b (lines 6208-6332)
**Spec ID:** `round3-phase06-spec11b-live-presence-component`
**Branch:** `forums-wave-continued` (per brief Section 1 — Eric handles all git ops; CC never commits/pushes/branches/rebases)
**Date:** 2026-05-15
**Brief:** `_plans/forums/spec-6-11b-brief.md`

---

## Affected Frontend Routes

Routes that render the PresenceIndicator OR that must enforce the crisis-flagged-page suppression rule (Gate-G-CRISIS-SUPPRESSION). Plan-recon R6 enumerates exhaustively; this list is the working baseline.

- `/prayer-wall` — feed; PresenceIndicator embedded in feed header, suppressed if any rendered post has `crisisFlag: true`
...
- `/settings` — adds the "Count me as present when I'm reading" toggle (protected route)
- `/prayer-wall/answered` — referenced by brief as a possible Answered Wall route; **not in the canonical route inventory** (`.claude/rules/12-project-reference.md`); plan-recon R6 confirms whether this route exists, and if so, whether suppression is needed (the brief notes answered posts by definition shouldn't be crisis-flagged, but the rule applies for completeness)
