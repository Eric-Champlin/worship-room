...
**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 7.6 (lines 6468–6490, tracker row 98)
**Spec ID:** `round3-phase07-spec06-friends-pin-to-top`
**Phase:** 7 (Cross-Feature Integration) — sixth spec, follows 7.1 ✅ + 7.2 ✅ + 7.3 ✅ + 7.4 ✅ + 7.5 ✅
**Branch:** `forums-wave-continued` (no branch switch — Phase 6 / 7.1 / 7.2 / 7.3 / 7.4 / 7.5 discipline)
**Date:** 2026-05-16
**Size:** L
**Risk:** Medium
**Prerequisites:** Spec 7.5 (✅ shipped per tracker row 97)
**Tier:** Standard

**Brief Source:** `_plans/forums/spec-7-6-brief.md`, authored 2026-05-16 against the live master plan stub + live code recon.
...
**Risk rationale:** Backend feed query enhancement is the meat. The work modifies the main Prayer Wall feed path (`PostService.listPosts` or equivalent — see recon), introduces a per-viewer ordering hint, adds a chip to PrayerCard, and adds a DTO field. Risk is bounded because: (a) `friend_relationships` table + entity + repository all exist since Phase 2.5.x; (b) 7.4's `FriendPrayersService.java` JUST shipped a friend-id lookup pattern that 7.6 can reuse; (c) `PostVisibility` enum (`FRIENDS`/`PUBLIC`/`PRIVATE`) and `PostSpecifications.visibleTo(viewerId)` predicate ALREADY exist and are wired into the existing query path; (d) PrayerCard has multiple existing chip patterns to mirror (`ScriptureChip`, `CategoryBadge`, `QotdBadge`). The genuine new work is the feed-ordering logic that hoists friend posts to position 0-2 in the result set without duplicating them in the chronological remainder.

---

## Spec-Time Recon Addendum (2026-05-16)

All brief recon verified on disk at spec time. Findings worth surfacing for plan-recon:

**1. DTO file name correction — `PostDto.java`, NOT `PostResponse.java`.** Brief asserts the field gets added to `backend/src/main/java/com/worshiproom/post/dto/PostResponse.java`. That file does NOT exist on disk. The actual feed-response DTO is `backend/src/main/java/com/worshiproom/post/dto/PostDto.java` (verified via `find`). A `CreatePostResponse.java` exists but is for the create-response wrapper only. Plan-recon should target `PostDto.java` (and `PostListResponse.java` if needed for the list-shape wrapper). The `PostMapper.java` reference is correct and lives at `backend/src/main/java/com/worshiproom/post/PostMapper.java` (not under `dto/`).

**2. `PostSpecifications.visibleTo(viewerId)` is the active predicate at three call sites.** Verified at `PostService.java:148`, `:170`, `:189` — three list paths all compose with `PostSpecifications.visibleTo(viewerId).and(...)`. Confirms 7.4's R2 question retroactively: visibility is ACTIVE in the codebase. The brief's assertion stands; the line number for the principal listPosts call is `:170` (the `:148` site is the cached spec branch per the comment at line 139).

**3. `isFromFriend` field is genuinely net-new.** `grep -rn isFromFriend backend/src/main/java/ frontend/src/` returns zero matches. Safe to add as a fresh DTO field with no migration risk.

**4. Tracker status verified.** 7.4 ✅ + 7.5 ✅ + 7.6 ⬜ per `spec-tracker.md:210-212`. Prerequisites cleanly satisfied.

**No other recon overrides.** All other brief claims verified:
- `FriendPrayersService.java` exists at `backend/src/main/java/com/worshiproom/post/FriendPrayersService.java` (alongside `FriendPrayersRateLimitConfig.java` + `FriendPrayersRateLimitService.java` + `FriendPrayersRateLimitedException.java`).
- `PostController.java`, `PostRepository.java`, `PostMapper.java`, `AnsweredFeedCache.java` all confirmed.
- `friend_relationships` Liquibase changeset shipped Phase 2.5.x (per 7.4 recon citation).
...
- `backend/src/main/java/com/worshiproom/post/PostService.java` (feed query enhancement — `listPosts` flow at the line :170 call site; cached spec at :148 also needs awareness)
- `backend/src/main/java/com/worshiproom/post/dto/PostDto.java` (add `isFromFriend` field — **corrected from brief's `PostResponse.java`** per Recon Addendum item 1)
- `backend/src/main/java/com/worshiproom/post/PostMapper.java` (compute `isFromFriend` during DTO mapping — plan-recon verifies the exact insertion point)
...
| `PostSpecifications.visibleTo(viewerId)` is the existing visibility predicate | ✅ | `PostService.java:170` (principal call site, with `:148` cached spec and `:189` answered spec also using the same predicate). Documented at `PostSpecifications.java:40-53` |
...
| `PostDto.java` is the actual feed-DTO file name (NOT `PostResponse.java`) | ✅ | Verified via `find backend/src/main/java/com/worshiproom/post -name '*Response*' -o -name '*Dto*'` — `dto/PostDto.java` exists; `dto/PostResponse.java` does NOT. `CreatePostResponse.java` exists but is the create-wrapper, not the feed item DTO. See Recon Addendum item 1 |
| `isFromFriend` field is genuinely net-new | ✅ | `grep -rn isFromFriend backend/src/main/java/ frontend/src/` returns zero matches. Safe new DTO field, no migration risk |

**Recon overrides:** Four things differ from the stub or the brief:
1. The master plan stub says "When the user has friends, recent posts from those friends pin to the top." It doesn't define **what counts as "recent"** — plan-recon picks a time window. Likely "last 24h" mirroring 7.4, but worth confirming.
...
4. **Brief asserts `dto/PostResponse.java` exists; on disk the file is `dto/PostDto.java`.** Plan-recon and execute targets `PostDto.java`. See Recon Addendum item 1.
...
- Lucide `Users` icon per master plan stub
...
## Acceptance Criteria (from master plan Spec 7.6)
...
- **`/spec-forums`:** Produces the actual spec file at `_specs/forums/spec-7-6.md` from this brief. ← (this file)
...
- **`/execute-plan-forums`:** Modify PostService.listPosts, add `isFromFriend` to `PostDto` (NOT `PostResponse` — see Recon Addendum item 1), create FromFriendChip component, integrate into PrayerCard. Pre-execution uncommitted-files check.
...
- `_plans/forums/spec-7-6-brief.md` — source brief
- `backend/src/main/java/com/worshiproom/post/PostService.java:120,148,170,189` — the feed query method to enhance, all three visibility-predicate call sites
- `backend/src/main/java/com/worshiproom/post/PostService.java:725-729` — PostVisibility enum coverage
...
- `backend/src/main/java/com/worshiproom/post/dto/PostDto.java` — **DTO to extend with `isFromFriend`** (NOT `PostResponse.java` — see Recon Addendum item 1)
- `backend/src/main/java/com/worshiproom/post/dto/PostListResponse.java` — list wrapper (verify shape during plan-recon)
- `backend/src/main/java/com/worshiproom/post/PostMapper.java` — DTO mapping location
...
- `_specs/forums/spec-7-1.md`, `_specs/forums/spec-7-2.md`, `_specs/forums/spec-7-3.md`, `_specs/forums/spec-7-4.md`, `_specs/forums/spec-7-5.md` — sibling Phase 7 specs
...
- `_forums_master_plan/round3-master-plan.md` line 8199 — Open Question #3: "Friends pin to top — exact ranking" (most-recent vs. most-recent-unprayed; default per MPD-1 is most-recent)
