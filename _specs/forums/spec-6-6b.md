...
**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 6.6 (`round3-phase06-spec06-answered-wall`, lines ~5625-5720). 6.6b has no independent stub — it is the remediation layer that brings shipped 6.6 into line with the live stub. See `_forums_master_plan/spec-tracker.md` line 165 for the drift-resolution note authored 2026-05-14.
**Brief Source:** `_plans/forums/spec-6-6b-brief.md` (preserved verbatim below as Sections 0–16)
**Branch:** `forums-wave-continued` (per the brief's Section 1 — CC never alters git state; Phase 6 specs accumulate on this branch — see commits `55630ce Spec 6-7`, `f9f2652 Spec 6-6`, `46d70a7 Spec 6-5`)
**Date:** 2026-05-14
...
**Size:** L (Large)
**Risk:** Medium-High — second permanent-MD5 CHECK-constraint migration on `post_reactions` (table already carries 6.6's shipped behavior fix), new reaction type wired through the full stack, load-bearing anti-pressure copy
**Tier:** **xHigh** — see brief Section 2 for the four factors

---

## Affected Frontend Routes

6.6b extends the shipped `/prayer-wall/answered` page (and its `AnsweredCard` composition that surfaces wherever an answered post renders). The `'celebrate'` reaction surfaces on answered posts on every Prayer Wall family route. The category filter chips surface only on the Answered Wall itself. `/verify-with-playwright` should cover:

- `/prayer-wall/answered` (PRIMARY — the Answered Wall: hero subhead fix, intro paragraph, AnsweredCategoryFilter chips, Celebrate reaction, missing-text fallback, Share-an-update + Un-mark affordances)
- `/prayer-wall/answered?category=health` (and `?category=family|work|grief|gratitude`) — chip-driven filter URL
- `/prayer-wall/:id` (answered post detail uses AnsweredCard variant — Celebrate reaction surfaces here too)
- `/prayer-wall/user/:id` (user profile's answered posts — Celebrate reaction surfaces here)
- `/prayer-wall/dashboard` (user's own answered prayers — Share-an-update + Un-mark affordances surface on author's own posts; Celebrate reaction available)

---

## Spec-time Recon Summary (verified on disk 2026-05-14)

The brief's R1–R2 are verified findings from the 2026-05-14 brief-drift audit; this summary re-confirms them on disk at spec-creation time and pre-fills several R3-R8 items the brief deferred to plan-time. The remaining R3-R8 plan-time recon is still required and explicitly listed below.

| Item | Status | Evidence |
|---|---|---|
| Shipped first ALTER changeset (the `'praising'` widening) | ✅ | `backend/src/main/resources/db/changelog/2026-05-14-001-add-praising-to-post-reactions-check.xml` — append-only ALTER, runs after `2026-04-27-016`. The 6.6b second ALTER must NEVER edit this file or `2026-04-27-016` (W1, W2). |
| `posts.is_answered`, `posts.answered_at`, `posts.answered_text` columns | ✅ | Verified at 6.6 spec-time (`spec-6-6.md` recon summary). `is_answered + answered_at` are CHECK-coupled. `answered_text` is the nullable column 6.6b's "optional text" handling targets. |
| Shipped AnsweredWall page | ✅ | `frontend/src/pages/AnsweredWall.tsx` exists (the surgical-edit target — Gate-G-EXTEND-NOT-DUPLICATE) |
| Shipped AnsweredCard component | ✅ | `frontend/src/components/prayer-wall/AnsweredCard.tsx` exists (the surgical-edit target for missing-text fallback, Share-an-update, Un-mark, and the Light-a-Candle→Celebrate swap) |
| Shipped wrong hero subhead | ✅ confirmed at spec time | `frontend/src/pages/AnsweredWall.tsx:88` and `frontend/src/constants/answered-wall-copy.ts:21` both contain the wrong string `'Prayers the community has watched God move in.'`. MPD-1 replaces this with `'Gratitude, not comparison.'` |
| Shipped MarkAsAnsweredForm (Spec 3.5 origin) | ✅ | `frontend/src/components/prayer-wall/MarkAsAnsweredForm.tsx` — plan-recon R3 reads this in full to determine the answered-text edit / un-mark gap |
| Shipped INTERCESSION regression tests (Gate-G-CELEBRATE-NO-INTERCESSION load-bearing) | ✅ | `backend/src/test/java/com/worshiproom/post/engagement/ReactionWriteIntegrationTest.java` — `togglePraying_addPath_firesIntercessionActivity` (L261), `togglePraising_addPath_doesNotFireIntercessionActivity` (L424), `toggleCandle_addPath_doesNotFireIntercessionActivity` (L441). **Naming note: brief refers to these without the `Activity` suffix (e.g., "`toggleCandle_addPath_doesNotFireIntercession`"); the actual JVM method names all end in `Activity`. Planner uses the actual method names verbatim — no rename of shipped tests permitted (Gate-G-6.6-REGRESSION-SAFE).** |
| Redis cache infrastructure (5.6 prereq for MPD-5) | ✅ | `backend/src/main/java/com/worshiproom/cache/CacheConfig.java`, `cache/RedisConfig.java`, `cache/CircuitBreakingCacheManager.java`. R6 is satisfied; plan reads these to mirror the established pattern. **No STOP condition (brief's "5.6 unavailable" branch does not trigger).** |
| `posts.category` storage (R7 prereq for MPD-3) | ✅ pre-filled | `backend/src/main/resources/db/changelog/2026-04-27-014-create-posts-table.xml:33,102-103` — `category VARCHAR(20)` with CHECK allowing `'health'`, `'mental-health'`, `'family'`, `'work'`, `'grief'`, `'gratitude'`, `'praise'`, `'relationships'`, `'other'`, `'discussion'`. **Brief's five chip labels (Health, Family, Work, Grief, Gratitude) map cleanly to existing values — no rename, no taxonomy invention. W22 STOP condition NOT triggered.** Plan-recon R7 still confirms the existing feed-query filter mechanism. See "Spec-time clarifications" below for the broader-taxonomy question. |
| Tracker status — 6.6 ✅ shipped & merged | ✅ | `_forums_master_plan/spec-tracker.md:179` shows ✅; tracker line 165 records the 2026-05-14 drift-resolution note authored against `_plans/forums/spec-6-6b-brief.md` |
| Spec file `_specs/forums/spec-6-6b.md` | ⬜ before this run | safe to create |

---

### Spec-time clarifications surfaced for the planner

Three items merit explicit planner attention beyond the brief's R3-R8 list.

**S1. Method-name discrepancy in shipped INTERCESSION regression tests (load-bearing for Gate-G-CELEBRATE-NO-INTERCESSION).**
The brief consistently writes the shipped test method names as `toggleCandle_addPath_doesNotFireIntercession`, `togglePraising_addPath_doesNotFireIntercession`, and `togglePraying_addPath_firesIntercessionActivity`. The first two are typos — the actual JVM method names are `toggleCandle_addPath_doesNotFireIntercessionActivity` and `togglePraising_addPath_doesNotFireIntercessionActivity` (the `Activity` suffix is present). The third (`togglePraying_addPath_firesIntercessionActivity`) matches. **This is a typo in the brief, not a real divergence. The planner MUST use the actual method names verbatim in the plan, and the new regression test (T5) for `'celebrate'` MUST follow the actual naming convention: `toggleCelebrate_addPath_doesNotFireIntercessionActivity`.** Gate-G-6.6-REGRESSION-SAFE prohibits renaming any shipped test, so the discrepancy resolves to "use actual names."

**S2. Post category taxonomy is broader than the chip set — MH omission still works, but what about `praise`/`relationships`/`other`/`discussion`?**
The DB's `posts.category` CHECK constraint allows ten values. The brief's five chips map to five of them (Health, Family, Work, Grief, Gratitude). The deliberate Mental Health omission (Gate-G-MH-OMISSION) covers a sixth (`mental-health`). The remaining four — `praise`, `relationships`, `other`, `discussion` — are NOT exposed by any chip in the brief's design and are NOT mentioned in the brief at all. The brief's wording ("the five chips map to existing stored category values") is consistent with the design but leaves these four unaddressed. **Planner: explicitly resolve the `praise`/`relationships`/`other`/`discussion` bucket. Plausible resolutions:** (a) they roll into "All" only and are NOT individually filterable — most consistent with brief intent and the live stub's chip list, and matches the MH treatment; (b) they get their own chips, growing the set beyond five — this contradicts the brief and the live stub; (c) plan surfaces to Eric for a sign-off. Default to (a) unless plan-recon R7 surfaces a contradiction. NOTE: `discussion` posts are NOT prayer posts at all (per `02b62f1`-era taxonomy); they cannot be answered (no `is_answered=true` discussion posts should exist), so they are effectively irrelevant to the Answered Wall query — verify this assumption at plan-recon R7.

**S3. R6 (Redis cache) and R7 (post category) are pre-confirmed at spec-time but the brief still lists them as plan-recon items.**
The brief's R6 says "if 5.6 is somehow not available, plan STOPS and surfaces." Spec-time recon confirms it IS available. The brief's R7 says "the five chips must map to real stored values." Spec-time recon confirms they DO map (`health`, `family`, `work`, `grief`, `gratitude` all exist). The planner still performs the deeper plan-recon work — read the existing cache pattern in detail (R6), confirm the existing feed-query filter mechanism uses the `category` column (R7) — but the brief's STOP conditions on either item should not fire.

---

### Items deferred to plan-time recon (brief Section 5 R3-R8)

- **R3** (PLAN-RECON-REQUIRED) — Read `MarkAsAnsweredForm.tsx` (Spec 3.5) and the shipped 6.6 answered-text rendering to determine the optional/edit/un-mark gap: is `answered_text` optional at the DB level only or also at the form/API level? Is there an existing edit path? Is there any un-mark capability anywhere? Determines how much of MPD-4 is net-new vs. wiring-up-existing.
- **R4** (PLAN-RECON-REQUIRED) — Re-grep the shipped codebase for ALL `reaction_type` enumeration sites (CHECK constraint, backend enum/constants, frontend type union, validation, switch/match, denormalized counter columns if any like `praying_count`/`candle_count`/`praising_count`). Document the complete list; `'celebrate'` must land at every site (W7). If a per-type counter pattern exists, `'celebrate'` needs `celebrate_count` following it (W8) — this is a sibling column-add in the migration.
- **R5** (PLAN-RECON-REQUIRED) — Read shipped `AnsweredWall.tsx` to find the exact hero subhead element to replace (MPD-1) and where the intro paragraph + `AnsweredCategoryFilter` mount. Read shipped `AnsweredCard.tsx` to find where the missing-text fallback and "Share an update" / un-mark affordances attach, and how the reaction bar is composed so the Light-a-Candle→Celebrate swap is surgical.
- **R6** (PLAN-RECON-REQUIRED, spec-time pre-confirmation: Redis IS available) — Read the existing cache patterns in detail and mirror them for the answered-feed cache (key shape, TTL handling, invalidation hooks).
- **R7** (PLAN-RECON-REQUIRED, spec-time pre-confirmation: `posts.category` IS the storage) — Read how the existing main Prayer Wall feed filters by category so the answered feed's `?category=` reuses the same mechanism. Verify the `praise`/`relationships`/`other`/`discussion` resolution from S2 above.
- **R8** (PLAN-RECON-REQUIRED) — Read `2026-05-14-001-add-praising-to-post-reactions-check.xml` in full to confirm: exact changeset id, whether it used drop-and-recreate or in-place constraint alter, what shape the second `'celebrate'` ALTER should take to be consistent (D-Migration2 commits to "same constraint-alter approach the first migration used" — R8 confirms what that approach is).

---

## Brief (preserved verbatim)

The remainder of this file is the verbatim content of `_plans/forums/spec-6-6b-brief.md`. The header, route inventory, and recon summary above are this skill's additions; everything below is Eric's authored brief.
