# Forums Wave: Spec 6.8 — Verse-Finds-You

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 6.8 (`round3-phase06-spec08-verse-finds-you`)
**Brief Source:** `_plans/forums/spec-6-8-brief.md` (preserved verbatim below)
**Branch:** `forums-wave-continued` (per Brief §1 — CC never alters git state at any phase; Phase 6 specs accumulate on this branch — see commits `c78843b Spec 6-6b`, `55630ce Spec 6-7`, `f9f2652 Spec 6-6`, `46d70a7 Spec 6-5`)
**Date:** 2026-05-14
**Phase:** 6 (Slow Sanctuary / Quiet Engagement Loop)
**Size:** L (Large)
**Risk:** HIGH (master plan) — AI-adjacent feature surfacing scripture into a user's emotional/spiritual state; mis-applied scripture in a vulnerability moment causes real harm
**Tier:** xHigh (brief §3) — HIGH-risk per master plan; failure mode is "wrong verse on a grieving/distressed person"; runtime-gated crisis-classifier dependency requiring a built-now-but-dormant-until-Phase-10 seam; 180-passage curated set is a human pastoral-discernment deliverable, not a code task; silent failure is a load-bearing discipline; "NOT hope" mappings and Redis-fail-closed cooldown are deliberate pastoral decisions that must ship with embedded rationale

---

## Affected Frontend Routes

Verse-Finds-You attaches to three trigger surfaces on the Prayer Wall and adds a settings toggle. `/verify-with-playwright` should cover all four routes — verifying the toggle defaults OFF, that no verse surfaces when OFF, and that an enabled trigger surfaces a verse with the correct prefix copy (driven against the seeded/mocked crisis-flag interface per Brief §0, since the feature is functionally dormant in true prod until Phase 10).
...
- `/prayer-wall/dashboard` (reading-time trigger may apply here if active-engagement signal counts user's own feed views — Plan-Recon R6 to confirm scope)
- `/settings` (the "Verse Finds You" toggle in a "Gentle extras" section; defaults OFF for new AND existing users)

---

## Spec-time Recon Summary (verified on disk 2026-05-14)

Brief's R1-R10 are designated PLAN-RECON-REQUIRED — the planner runs them at `/plan-forums` time. This summary captures spec-time filesystem reality so the plan can build on confirmed facts rather than re-discover them.

| Item | Status | Evidence |
|---|---|---|
| `com.worshiproom.verse` Java package | ❌ does not exist | `ls backend/src/main/java/com/worshiproom/` → 18 packages, none named `verse`. Safe to create. |
| `backend/src/main/resources/verses/` directory | ❌ does not exist | Brief §11 creates it; safe. |
| `backend/src/main/resources/verses/verse-finds-you.json` | ❌ does not exist | Gate-G-CURATION-PREREQ is ACTIVE — Eric's 180-passage deliverable must be ready and reviewed before the execute steps that build/run the JSON validation tests. CC never generates placeholder verses. |
| Crisis-detection scaffolding (R4 — pre-existing crisis classifier seam) | ✅ EXISTS | `backend/src/main/java/com/worshiproom/post/Post.java:66-67` defines `crisis_flag` column and `crisisFlag` field. `PostService.java:300` invokes `PostCrisisDetector.detectsCrisis(detectionInput)` on post create. `PostService.java:519-524` carries the "once flagged, stays flagged" rule on edit. **Plan-Recon R4 implication:** the per-user crisis-flag read (Step 1 of selection — "if user has triggered crisis-flag detection in the last 48h, return null") wraps a query against `Post.crisisFlag` filtered by `(user_id, created_at within last 48h)`. The seam is real and built-on; it is NOT a green-field interface. Plan must specify the exact `CrisisFlagGate` (or equivalent) shape that reads through this existing scaffolding. The pre-Phase-10 dormancy described in Brief §0 still applies: Phase 10 (10.5 routing + 10.6 automated flagging) will widen the inputs the gate considers; until then, Step 1 reads from the existing per-post `crisis_flag` for the requesting user's posts. |
| 6.7 `imageGen.ts` (R2 — Save-this-verse reuse target) | ✅ EXISTS | `frontend/src/lib/prayer-wall/imageGen.ts` — confirmed; 6.7 narrowed the pipeline. Consume-don't-extend per D-SaveReuse6.7. |
| Trigger insertion points (R6) | ⚠️ partial | `frontend/src/components/prayer-wall/InlineComposer.tsx` ✅; `frontend/src/components/prayer-wall/CommentInput.tsx` ✅; **brief names `frontend/src/components/prayer-wall/PrayerWallFeed.tsx` but that exact file does NOT exist** — the reading-time trigger lives in `frontend/src/pages/PrayerWall.tsx` (page-level) or a feed component within it. Plan-Recon R6 reconciles: find the right active-engagement signal hook point in the page or its child components, and document the actual file modified. |
| `wr_settings.*` nesting convention | ✅ confirmed | `.claude/rules/11-local-storage-keys.md:60` documents `wr_settings.prayerWall.{prayerReceiptsVisible, nightMode, watchEnabled, dismissedShareWarning}`. `wr_settings.verseFindsYou.enabled` fits the established shape exactly. Plan-Recon R10 documents both new keys (`wr_settings.verseFindsYou.enabled` and `wr_verse_dismissals`). |
| Liquibase changeset slot (R8) | ⚠️ brief proposal stale | Brief proposes `2026-04-22-003-create-verse-surfacing-log.xml`. The most recent changeset on disk is `2026-05-14-004-add-posts-celebrate-count-column.xml` (shipped with 6.6b on 2026-05-14). The 2026-04-22 slot is far behind current sequence. Plan-Recon R8 picks the correct next available slot at execute time — likely `2026-05-14-005-create-verse-surfacing-log.xml` or a later date if subsequent changesets land before 6.8 executes. Table shape (`(user_id, verse_id, surfaced_at, trigger_type, dismissed_at)`, index on `(user_id, surfaced_at DESC)`, 30-day retention) is what matters, not the literal filename. |
| 5.6 Redis cache (R5 — cooldown infrastructure) | ⚠️ confirm at plan time | Not verified at spec time. Brief §17 marks 5.6 confirmed. Plan-Recon R5 verifies Redis is wired and reads existing Redis patterns to implement Brief §D-FailClosed (cooldown check defaults to DENY when Redis is unreachable). If 5.6 is NOT available, plan-recon STOPS and surfaces per Brief §15. |
| Settings "Gentle extras" section (R7) | ⚠️ confirm at plan time | Not verified at spec time. Plan-Recon R7 determines whether a "Gentle extras" section exists (e.g. from 6.3 Night Mode or 6.4 Watch) or must be created here. The "Verse Finds You" toggle defaults OFF for new AND existing users — plan confirms the settings-default mechanism handles existing users correctly per Gate-G-DEFAULT-OFF. |
| Scheduled-job mechanism for 30-day retention (R9) | ⚠️ confirm at plan time | Not verified at spec time. Plan-Recon R9 determines whether Spring `@Scheduled` or an equivalent retention-purge pattern already exists in the backend that the `verse_surfacing_log` purge hooks into. |
| Post category storage (R3 — nine-category mapping) | ⚠️ confirm at plan time | Plan-Recon R3 reads how post category is stored/read (the same question 6.6b's R7 asked). The nine categories in the brief's mapping (`health`, `mental-health`, `family`, `work`, `grief`, `gratitude`, `praise`, `relationships`, `other`) must map to real stored category values. If the mapping doesn't align cleanly, plan-recon STOPS and surfaces per Brief §15. |
| 180-passage curated JSON (R1 — gate readiness) | ❌ NOT READY (active gate) | The file does not exist yet. Per Brief §2 and Gate-G-CURATION-PREREQ, this is Eric's deliverable, completed and reviewed BEFORE execute reaches the validation-test step. Plan may plan the machinery (schema, header, mapping block, engine, validation tests); the *execute* steps that run the validation tests against the real file are gated on file readiness. |

**Recon override:** none. The brief does not propose creating anything that already exists in conflict (the existing crisis-detection scaffolding informs R4's seam shape; it does not collide with a new green-field interface).
