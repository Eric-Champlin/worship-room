# Forums Wave: Spec 6.6 — Answered Wall

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 6.6 (`round3-phase06-spec06-answered-wall`)
**Brief Source:** `_plans/forums/spec-6-6-brief.md` (preserved verbatim below as Sections 1–16)
**Branch:** `forums-wave-continued` (per W1 — CC never alters git state; Phase 6 specs accumulate on this branch — see commits `46d70a7 Spec 6-5`, `d4ccbca Spec 6-4`, `6f363eb Spec 6-3`)
**Date:** 2026-05-14
**Tier:** xHigh (low-end) — DB migration on a permanent-MD5 CHECK constraint; new public-facing surface; brand-defining emotional weight. Not MAX: no crisis/privacy/AI stakes, and the data model + mark-as-answered flow already exist (R1–R3 verified).

---

## Affected Frontend Routes

The new page is `/prayer-wall/answered`. The AnsweredCard variant inherits PrayerCard, so the answered-variant rendering and the `'praising'` reaction surface wherever an answered post renders today. The plan-time recon (R5) determines the canonical nav-entry layout; the routes below are the candidate surfaces `/verify-with-playwright` should cover.

- `/prayer-wall/answered` (NEW — the Answered Wall page itself)
- `/prayer-wall` (navigation entry "Answered" added; main feed continues to default to non-answered behavior per W26)
- `/prayer-wall/:id` (answered post detail uses AnsweredCard variant)
- `/prayer-wall/user/:id` (user's answered posts on their profile)
- `/prayer-wall/dashboard` (user's own answered prayers shown with answered variant)

---

## Spec-time Recon Summary (verified on disk 2026-05-14)

Brief's R1–R4 verified during brief authoring. This summary re-confirms on disk at spec-creation time, surfaces the load-bearing master-plan divergence (MPD-1), and flags items deferred to plan-time (R5–R10).

| Item | Status | Evidence |
|---|---|---|
| `posts.is_answered` BOOLEAN column | ✅ | `2026-04-27-014-create-posts-table.xml:44` — `NOT NULL DEFAULT FALSE`; `Post.java:53-54` maps `isAnswered` field |
| `posts.answered_text` TEXT column | ✅ | `2026-04-27-014-create-posts-table.xml` (referenced); `Post.java:56-57` maps `answeredText` |
| `posts.answered_at` TIMESTAMPTZ column | ✅ | `Post.java:59-60` maps `answeredAt` |
| CHECK constraint coupling `is_answered` + `answered_at` | ✅ | `2026-04-27-014-create-posts-table.xml:123` — `(is_answered=FALSE AND answered_at IS NULL) OR (is_answered=TRUE AND answered_at IS NOT NULL)` |
| Partial index `idx_posts_is_answered_created` | ✅ | `2026-04-27-014-create-posts-table.xml:178-180` — `(is_answered, answered_at DESC) WHERE is_answered=TRUE AND is_deleted=FALSE` |
| `post_reactions` table + CHECK `IN ('praying','candle')` | ✅ | `2026-04-27-016-create-post-reactions-table.xml:25, 54-60` — comment explicitly anticipates the Phase 6.6 ALTER widening to add `'praising'` and `'celebrate'` |
| `PostReaction.java` JPA entity (`reaction_type` String) | ✅ | `backend/src/main/java/com/worshiproom/post/engagement/PostReaction.java:35` |
| `ReactionWriteService` with `PRAYING`/`CANDLE` private constants + switch-shaped logic | ✅ | `backend/src/main/java/com/worshiproom/post/engagement/ReactionWriteService.java:46-47, 163-173` |
| `EngagementService` with `PRAYING`/`CANDLE` private constants | ✅ | `backend/src/main/java/com/worshiproom/post/engagement/EngagementService.java:38-39, 54-57` |
| `ReactionRepository` comment references both types | ✅ | `backend/src/main/java/com/worshiproom/post/engagement/ReactionRepository.java:17` |
| Frontend `ReactionTypeApi = 'praying' \| 'candle'` union type | ✅ | `frontend/src/types/api/prayer-wall.ts:42` |
| Frontend `reactionsStore.ts` hardcoded literals | ✅ | `frontend/src/lib/prayer-wall/reactionsStore.ts:464, 541` (toggleReaction calls with `'praying'`, `'candle'`) |
| Frontend `prayer-wall-api.ts` reaction toggle + remove signatures use `ReactionTypeApi` | ✅ | `frontend/src/services/api/prayer-wall-api.ts:470, 495` |
| 6.5 spec file (prerequisite — Intercessor Timeline) | ✅ on disk | `_specs/forums/spec-6-5.md` exists; 6.5 execute is in-flight per recent commits |
| 6.5 merged to master | ⏳ | Tracker shows ⬜ for 6.5; recent commits indicate 6.5 is in-progress on `forums-wave-continued` but not yet on master. **6.6 execute must wait for 6.5 merge per Section 16 — this spec can be authored now, executed later.** |
| Prayer Wall Redesign side quest | ⏳ | Recommended-first (MPD-2). Sequencing decision is at execute time (R5). |
| Spec file `_specs/forums/spec-6-6.md` | ⬜ before this run | safe to create |

**Master-plan divergences surfaced at spec time (planner attention required):**

- **MPD-1 (load-bearing):** Brief adds ONLY `'praising'`. Master plan stub at line 5630 + line 5707 specifies BOTH `'praising'` AND `'celebrate'`, and references a `CelebrateReaction.tsx` component file at line 5685, distinct reaction-count rendering ("12 praising with you · 3 celebrating") at line 5631, and a "Celebrate" reaction label in the Copy Deck at line 5731. Brief's rationale (Section 4): `'celebrate'` has no consumer in the brief's narrower 6.6 scope, so adding the enum value is speculative schema. **The brief is authoritative per the brief's own Section 4 directive ("Plan/execute MUST honor the brief, not the stub"), but the planner MUST surface this as a Plan-Time Divergence requiring Eric's explicit sign-off before execute** — narrowing scope from "two new reaction types + two new label-swaps + new CelebrateReaction component" to "one new reaction type + one new label" materially changes acceptance criteria, file lists, and the brand-voice surface (the master plan's "Light a Candle → Celebrate" swap on answered posts disappears under the brief's scope). If Eric reaffirms the brief's narrowing at plan review, the Liquibase changeset, OpenAPI doc, and tests all narrow to `'praising'` only. If Eric re-aligns to the master plan, all four values (`'praying'`, `'candle'`, `'praising'`, `'celebrate'`) ship in the same changeset and the CelebrateReaction work returns to scope.

- **MPD-related semantic flag (R9 prefilled):** Brief leans `answered_at IS NOT NULL` as the canonical "is answered" filter. **Schema reality: `posts.is_answered` is a real BOOLEAN column with a CHECK constraint coupling it to `answered_at`** (changeset 014:123). The two are functionally equivalent because of the CHECK, but the schema's partial index `idx_posts_is_answered_created` is built on `WHERE is_answered = TRUE` (changeset 014:180). **Recommendation for plan:** filter on `is_answered = TRUE` to use the partial index; ordering uses `answered_at DESC` per brief D-AnsweredFilter. This is a query-shape clarification, not a divergence — the brief deferred this to R9, and the disk verifies the canonical filter shape.

- **MPD-1 collateral:** Brief's file list (Section 10) omits `CelebrateReaction.tsx` and `AnsweredWallHero.tsx` / `AnsweredCategoryFilter.tsx` that the master plan stub specifies. The brief reduces the page structure to a chronological feed without category filter chips, no Mental-Health-omission rationale callout, and no hero subhead "Gratitude, not comparison." The brief's D-Copy provides its own subhead ("Prayers the community has watched God move in.") — these are intentional copy and structural divergences, not omissions. **Planner: surface all three as Plan-Time Divergences alongside MPD-1.**

**Items deferred to plan-time recon (R5–R10 per brief Section 5):**
- R5: Current Prayer Wall navigation structure (redesign side quest status at execute time)
- R6: Answered Wall feed endpoint approach — query param on existing feed vs. dedicated `/api/v1/posts/answered` endpoint (master plan stub at line 5713 specifies the dedicated endpoint; brief leans query param — another Plan-Time Divergence the planner must reconcile)
- R7: `'praising'` reaction on non-answered posts — DB-permissive (brief lean) vs. server-enforced
- R8: AnsweredCard extension mechanism — prop variant vs. wrapper vs. shared-inner-component (master plan stub at line 5697 specifies prop-variant: "accept `answeredVariant` prop" on PrayerCard — recommend planner default to this unless PrayerCard's current shape contraindicates)
- R9: Canonical "is answered" filter — pre-filled above; planner confirms `is_answered=TRUE` choice
- R10: Every reaction-type enumeration site (DB constraint, backend `ReactionWriteService.PRAYING/CANDLE`, `EngagementService.PRAYING/CANDLE`, frontend `ReactionTypeApi` union, `reactionsStore.ts` literals, `prayer-wall-api.ts`) — pre-enumerated above; planner verifies completeness and identifies any switch/match statements over reaction types
- W14 (counter): Whether existing reactions use a per-type denormalized counter on `posts` requiring a matching column for `'praising'` (and `'celebrate'` if MPD-1 reverses)

---

## Spec Body (verbatim from brief — Sections 1–16)

The remainder of this file is the brief content authored at `_plans/forums/spec-6-6-brief.md`, preserved verbatim. The brief is the authoritative source for plan + execute per its own Section 4 directive. The Spec-time Recon Summary above is the only addition.
