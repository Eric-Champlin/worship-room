# Forums Wave: Spec 4.3 — Testimony Post Type

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 4.3
...
The testimony composer surface is `/prayer-wall`. PrayerCard chrome and the Sparkles TypeMarker propagate to every site that renders a PrayerCard:

- `/prayer-wall` — PrayerWall.tsx (composer + feed). The production composer entry point for testimony ships in Spec 4.7 (Composer Chooser); 4.3 exposes the variant via a temporary `?debug-post-type=testimony` query param for `/verify-with-playwright` only, marked `REMOVE-IN-4.7` in the code.
- `/prayer-wall/:id` — PrayerDetail.tsx (single-card render)
...
All routes except `/prayer-wall/dashboard` and `/my-prayers` are public. There are no new routes; this spec only modifies what already-existing routes render when a post has `postType='testimony'`.
