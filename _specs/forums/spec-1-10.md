# Forums Wave: Spec 1.10 — Phase 1 Cutover and End-to-End Test

**Master Plan Reference:** `_forums_master_plan/round3-master-plan.md` → Spec 1.10 (v2.9, Phase 1)
...
This spec adds **no frontend code** — it produces a Playwright E2E test, a Railway cutover runbook, and a CLAUDE.md update. `/verify-with-playwright` is NOT the right verification pass here; the authored `phase01-auth-roundtrip.spec.ts` IS the verification. It exercises the real auth roundtrip against the already-shipped UI from Specs 1.9 and 1.9b.
...
Because this spec produces no visual UI changes, no capture harness updates, and no new components, the `/verify-with-playwright` skill should be skipped for this spec. The Playwright test's pass/fail against local dev (and later against prod Railway) is the full verification signal.
